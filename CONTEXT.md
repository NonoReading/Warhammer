...
**ARMURE A 0 PA SUR TOUS LES EMPLACEMENTS (26/09/2026, CAUSE IDENTIFIEE - PAS UN BUG DE CODE, A VERIFIER PAR NONO)** : signale par Nono sur Gunther Krieg (Cotte de maille `PERSO-ARMO_02` + Coiffe de maille `PERSO-ARMO_01`, catalogue custom `BOOK_PERSO.Xml`). **Catalogue et moteur de calcul innocentes** : `PERSO-ARMO_01`/`_02` ont bien `<ArmorPoint>"2"</ArmorPoint>` correctement rempli. Le calcul par emplacement (`pdfpersonnage.pas` ~L1942-1961, boucle `ArmureBras`/`ArmureCorps`/`ArmureJambe`/`ArmureTete` += `PArmure.Protection`) n'additionne que si `PersonnageEquipement.Porte` est vrai, et `Porte` (`chargepersonnage.pas` ~L1276) vaut `Assigned(Node.Attributes.GetNamedItem('worn'))` - simplement absent du XML pour ces 2 pieces (`<Item name="PERSO-ARMO_02">""</Item>`, pas de `worn="1"`), contrairement a une armure reellement portee ailleurs (ex. "Arabyan Nun" : `<Item name="RULES-ARMO_05" worn="1">...`). **Cause probable : la case "porte" de ces 2 armures n'est simplement pas cochee sur la fiche de Gunther dans WinPersonnage** - a verifier par Nono avant tout code. **Reste a faire si confirme non-coche** : rien, comportement attendu, juste cocher la case. **Si la case est bien cochee a l'ecran malgre le XML sans `worn="1"`**, ce serait alors un vrai bug d'ecriture XML propre a l'equipement custom du livre PERSO, a investiguer dans winpersonnage.pas (ecran d'equipement, gestion de la case "porte"). **Suite immediate (meme jour), demandee par Nono pour la visibilite** : les lignes d'arme/armure non "portees" sont desormais grisees, a la fois sur `TabEquipementAffichage` (`winpersonnage.pas`, `TabDrawCell` ~L4728, colonne 8 vide => `clGray`) et sur les 4 blocs PDF concernes (`pdfpersonnage.pas` : bloc arme et bloc armure de `PdfPersonnageCreation` ~L1786-2027, et `PdfBlocArmesDonnees`/`PdfBlocArmuresDonnees` de la fiche etendue ~L3472-3834, `SetColor(RGB(150,150,150))`/`SetColor(clBlack)` autour de chaque ligne). Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes). **A TESTER PAR NONO** sur Gunther Krieg (armure non cochee => ligne grisee sur l'ecran et sur le PDF). Voir `A FAIRE.txt` et `Log.txt` 26/09/2026.

**TALENT DE CARRIERE (MetierTalent) COMPTE "NIVEAU DE CARRIERE" RANGS AU LIEU DE 1 DANS LE CALCUL DE DEGATS/EFFETS (26/09/2026, compile 0 erreur, XML de Gunther Krieg corrige a la main, A TESTER PAR NONO)** : signale par Nono sur Gunther Krieg - la Gaffe (Boat Hook, `RULES-COMB_BASE_02`, degats `SB+4`) affichait `DR+14` au lieu de `DR+12` attendu apres avoir augmente Coup Puissant (`RULES-T0037`) de 1 a 2 sur l'ecran d'Augmentation. **Premiere piste explorée puis abandonnee** : stocker `1` au lieu de `PMetierTalent.NiveauMetier` dans `Personnage.MetierTalent` a l'entree en carriere (`winpersonnage.pas` ~L6379) - revertee, ce champ sert AUSSI de filtre ailleurs (`CalculAvancement` ~L5977 : `PersonnageTalent.Valeur = StrToInt(MetierNvEnCours)`, pour proposer les talents nouvellement debloques au palier atteint) ; l'ecraser a 1 aurait casse cette comparaison des le niveau 2 (plus aucun talent de carriere propose au-dela du niveau 1). **Cause reelle, confirmee par Nono** ("j'ai pris coup puissant par la carriere actuelle puis l'ai augmente au niveau 2") : `Personnage.MetierTalent[].Valeur` porte deliberement le NIVEAU DE CARRIERE ou le talent apparait dans la table (`StructureMetierTalent` n'a pas de champ "nombre de rangs" - `chargemetiertalent.pas`), pas un nombre de rangs - mais `PersonnageTalentModificateur` (`chargepersonnage.pas` ~L1889-1896, point de consommation pour ModifyDamage/ModifyCarac/ModifyWeapon/ModifyArmure) le multipliait quand meme par `Facteur` comme s'il s'agissait d'un rang. Invisible pour un talent a rang unique (Max=1, la plupart), un talent multi-niveaux (Max>1, ex. Coup Puissant SB+niveau) pris a un cran de carriere >=2 comptait directement 2/3/4 rangs, en plus des rangs achetes ensuite via l'Augmentation (`Personnage.AugmentationTalent`, boucle suivante de la meme fonction) sans recoupement. **Corrige au point de consommation** : la boucle `MetierTalent` de `PersonnageTalentModificateur` utilise desormais `Facteur*1` (un pick de carriere accorde toujours exactement un rang), au lieu de `Facteur*PersonnageTalent.Valeur` - le champ `Valeur`/`NiveauMetier` reste intact partout ailleurs (ecriture XML, filtre `CalculAvancement`). **XML de Gunther deja ecrit avec la donnee incoherente** : `CHAPTER_INCREASE/SUBCHAPTER_TALENT` `RULES-T0037` corrige a la main de `"2"` a `"1"` dans le dernier fichier (`20260926-062150.xml`) - avec la carriere qui compte desormais pour 1 rang fixe (au lieu de 2 avant la coche), le total voulu par Nono (2) = 1 (carriere) + 1 (augmentation), pas 1+2. `CHAPTER_TALENT/SUBCHAPTER_TALENTCAREER` laisse INCHANGE (les niveaux 1/2/3/4 d'origine, y compris Coup Puissant a "2" et Robuste a "4" - ce sont des niveaux de carriere, pas des rangs, ne plus les "corriger" a 1). Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes). **A TESTER PAR NONO** : rouvrir Gunther Krieg, Gaffe a `DR+12`. **Limite connue, non corrigee** : l'ecran Augmentation (`TabAugmentationTalent`, colonne "Actuel") lit le total affiche sur `TabTalent` (`ColTalNb`), qui ne fusionne QUE Creation+Augmentation - `Personnage.MetierTalent` n'y est jamais merge. Un talent multi-niveaux pris via la carriere reste donc INVISIBLE sur cet ecran (Coup Puissant y affichait niveau "1" avant la manip de Nono, alors que le vrai total avec la carriere etait deja 2) : le joueur doit mentalement ajouter les rangs de carriere pour viser le bon total en cochant "Nouveau". Chantier plus lourd (fusionner MetierTalent dans `TabTalent`/`ColTalNbCrea` et corriger le calcul de delta a la sauvegarde, ~L6204) - voir `A FAIRE.txt`. **Reste** : d'autres personnages avec un talent multi-niveaux pris via la carriere ET augmente peuvent porter le meme decalage - a corriger au cas par cas si Nono en repere (le talent affiche un total inferieur a la realite du moteur de calcul).
