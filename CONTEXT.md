...
**SCISSION DE L'EQUIPEMENT PAR CATEGORIE (24/09/2026, ETAPE 1 FAITE ET VALIDEE PAR NONO, ETAPE 2 A FAIRE)** : nait de la remarque de Nono en voulant restreindre les amenagements de bateau (ci-dessous) a la bonne taille de navire. `TabEquipement` (WinPersonnage) reste aujourd'hui un tableau UNIQUE et plat qui melange Armes/Armures/Divers/Sorts/Montures/Navires/Vehicules via une colonne Type cachee. **Etape 1, faite** : `PageExperience` (10 onglets Attribut/Competence/Talent/XP/Evolution/Historique/Livre/MjCost/Corruption/Mutation, "cela devient complique" selon Nono) convertie de `TPageControl` a `TNotebook` pilote par une `TListBox` laterale (`ListBoxExperience`, `winpersonnage.pas`/`.lfm`) - la liste choisit la page affichee, meme role que la bande d'onglets avant. `TNotebook` n'a ni `ShowTabs` ni `OnPageChanged` publies dans la LCL installee (verifie dans `C:\lazarus\lcl\extctrls.pp`) : proprietes retirees du `.lfm`, `Caption`/`ClientHeight`/`ClientWidth` des pages aussi (geres par ailleurs, Caption reaffecte par code dans `AfficheImageRace`). Symptome bloquant en cours de mise au point : texte des controles enfants (ex. `RadioButtonRAS`/`Suivant`/`Changer`) invisible sur fond noir - deux pistes fausses explorees (repaint force `Invalidate`, cablage direct de l'evenement de changement de page), la vraie cause etait juste la couleur de fond de `ListBoxExperience` a regler explicitement (`Color`, proche de `clNone`, resolu par Nono via l'IDE). **Lecon** : verifier la couleur avant de partir sur des pistes d'evenements/repaint. **Etape 2, a faire** : meme mecanisme liste+panneau, generalise avec un filtre par Type (pas un `TPageControl`/`TNotebook` a categories codees en dur), applique a `TabEquipement` pour scinder Montures/Navires du reste en premier, puis Armes/Armures/Divers plus tard sans tout refaire. Le "Confier a..."/double-clic Fiche doivent continuer a fonctionner entre grilles separees. Voir A FAIRE.txt et Log.txt 24/09/2026.
