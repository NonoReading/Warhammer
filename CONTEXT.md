...
**POINT DE REPRISE PROCHAINE SESSION (26/09/2026) : ARMURE A 0 PA SUR TOUS LES EMPLACEMENTS.** Signale par Nono en toute fin de session sur Gunther Krieg, pas encore investigue. Il porte une armure (Cotte de maille `PERSO-ARMO_02` + Coiffe de maille `PERSO-ARMO_01`, catalogue custom `BOOK_PERSO.Xml`, livre perso de Nono - pas le Rulebook), et chaque emplacement de protection (Tete/Corps/Bras/Jambes) affiche 0 PA sur la fiche alors qu'il porte bien une armure. Pistes de depart pour la prochaine session : (1) verifier que `PERSO-ARMO_01`/`_02` ont bien un champ Point d'Armure/AP par localisation correctement rempli dans `BOOK_PERSO.Xml` (catalogue custom, jamais audite comme les livres officiels) ; (2) si le catalogue est correct, regarder cote moteur de calcul (`CalculTotaux`/logique d'AP par localisation dans `winpersonnage.pas`, ou `PdfPersonnageArmure`/equivalent dans `pdfpersonnage.pas`) ; (3) comparer avec une armure du Rulebook standard sur le meme personnage pour isoler "probleme de donnees du catalogue PERSO" vs "bug du moteur de calcul general". Voir `A FAIRE.txt` et `Log.txt` 26/09/2026.
