...
**ECRAN DE CHOIX DES FABRICATIONS REGROUPE PAR THEME (25/09/2026, compile 0 erreur, A TESTER PAR NONO)** : Nono remarque que l'ecran WinFabrication melange a plat toutes les fabrications de tous les livres actifs (Lightweight/Fine/Durable du Rulebook, 71 runes naines...), rendant la liste illisible des que plusieurs livres sont actifs - le but demande est de "trouver plus facilement". Decision prise avec Nono : deduire le theme d'une fabrication de son CODE (`LIVRE-THEME_suffixe`, ex. `DWARF-RUNE_01` -> theme `RUNE`, `RULES-QUALITY_01` -> `QUALITY`, `RULES-DEFECT_01` -> `DEFECT`) plutot qu'un nouveau champ XML a saisir a la main sur les 71 runes existantes - convention deja respectee par toutes les entrees `DATA_CRAFTMANSHIP` du projet (Rulebook, Dwarf Player's Guide, Nations of Mankind, Horned Rat Companion). Nouvelle fonction `FabricationTheme(CodeFabrication)` (`chargefabrication.pas`) : tout ce qui suit le dernier `_` du code, apres le `SeparateurLivre`. `winfabrication.pas` (`FormCreate`) : nouvelle colonne "Theme" (`RULES-LAB_289`, ajoutee a `INTERFACE.Xml`/`INTERFACE_FRANCAIS.Xml`) en fin de grille (colonne 7, les indices existants 1/3/4/5/6 inchanges) ; la liste filtree est desormais triee par cle "theme|libelle" (`TStringList.Sorted`, l'index d'origine dans `ListFabrication` porte comme objet associe pour retrouver l'entree sans collision entre deux lignes de meme theme/libelle) avant remplissage de la grille, au lieu de l'ordre de chargement des livres. Voir aussi `CONTEXT.md` §1.1 (index des mecanismes "objet special" par theme, ajoute plus tot dans la meme session). Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes). **Reste a Nono** : ouvrir WinFabrication sur un objet d'un personnage avec plusieurs livres actifs (Rulebook + Dwarf Player's Guide par exemple) et verifier que la colonne Theme s'affiche et que le regroupement facilite la recherche.
