...
**SCISSION DE L'EQUIPEMENT PAR CATEGORIE (24/09/2026, chantier ouvert, A CONCEVOIR CE SOIR AVEC NONO)** : nait de la remarque de Nono en voulant restreindre les amenagements de bateau (ci-dessous) a la bonne taille de navire. `TabEquipement` (WinPersonnage) est aujourd'hui un tableau UNIQUE et plat (pas dans un onglet) qui melange Armes/Armures/Divers/Sorts/Montures/Navires/Vehicules via une colonne Type cachee - un vrai fourre-tout, releve par Nono lui-meme. Il veut a terme un onglet par categorie, sur le modele du `TPageControl` deja utilise par `PageExperience` (Attribut/Competence/Talent/XP/Evolution/Historique/Livre/MjCost/Corruption/Mutation) - mais `PageExperience` a lui-meme deja trop d'onglets ("cela devient complique", Nono 24/09/2026) : **a resoudre AVANT de dupliquer ce modele sur l'Equipement**, sinon meme probleme en pire (plus de categories a scinder qu'en Experience). A concevoir ensemble ce soir : (1) solution au trop-plein d'onglets de `PageExperience` ; (2) meme solution appliquee a l'Equipement, avec un mecanisme de filtrage par Type reutilisable (pas un `TPageControl` a onglets codes en dur par categorie) pour que Montures/Navires soit la premiere categorie scindee, puis Armes/Armures/Divers plus tard sans tout refaire ; (3) le "Confier a..."/double-clic Fiche doivent continuer a fonctionner entre deux grilles separees. Rien code. Voir A FAIRE.txt et Log.txt 24/09/2026.
