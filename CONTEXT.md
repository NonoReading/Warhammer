...
**AUDIT DES TXT (20/09/2026, point de reprise)** : les quatre livres a `COMPLETE=0` sont audites. Sylvania, Nations of Mankind et Dwarf Player s Guide passes a `COMPLETE=1` ; High Elf Player s Guide reste a 0 (Adapting Careers p.60-62, ligne Mage des tables de tirage, table p.59) et a recu 8 navires + Eagle Claw Bolt Thrower (a compiler et valider). Suite : auditer les livres a `COMPLETE=1` un par un ; methode : comparer chaque section du sommaire au XML et verifier par grep avant de dire qu une donnee manque (deux fausses alertes le 20/09 : montures de Nations, herbes de HELFG). Voir `A FAIRE.txt`.
