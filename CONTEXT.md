...
**TALENT PRIS PAR LA CARRIERE COMPTE POUR "NIVEAU DE CARRIERE" RANGS AU LIEU DE 1 (26/09/2026, compile 0 erreur, XML de Gunther Krieg corrige a la main, A TESTER PAR NONO)** : signale par Nono sur Gunther Krieg - la Gaffe (Boat Hook, `RULES-COMB_BASE_02`, degats `SB+4`) affichait `DR+14` au lieu de `DR+12` attendu apres avoir augmente Coup Puissant (`RULES-T0037`) de 1 a 2 sur l'ecran d'Augmentation. Diagnostic (comparaison des XML successifs, `SAVED_CARACTERS/WFRP4/Gunther Krieg`) : Coup Puissant existe EN DOUBLE dans le personnage - une fois dans `Personnage.MetierTalent` (talent pris via un choix de la table de carriere, fige a "2" depuis le 11/09/2026 au moins, bien avant la manip du jour) et une fois dans `Personnage.AugmentationTalent` (celui que l'ecran d'Augmentation modifie, passe de 1 a 2 le 26/09). `PersonnageTalentModificateur` (`chargepersonnage.pas`) additionne les deux listes sans les recouper : 2+2=4 rangs au lieu de 2, d'ou +14 au lieu de +12. **Cause racine, confirmee par Nono** ("j'ai pris coup puissant par la carriere actuelle puis l'ai augmente au niveau 2") : a l'entree en carriere (`winpersonnage.pas`, ~L6379-6390), `Personnage.MetierTalent` est reconstruit en bouclant TOUTE la table de talents de la carriere (`ListMetierTalent`, tous niveaux confondus, `StructureMetierTalent` n'a pas de champ "nombre de rangs" - `chargemetiertalent.pas`) et `PersonnageTalent.Valeur := PMetierTalent.NiveauMetier` stockait le NIVEAU DE CARRIERE ou le talent apparait dans la table (1/2/3/4) au lieu du nombre de rangs accordes par ce choix (toujours 1). Invisible pour un talent a rang unique (Max=1, la plupart), mais un talent multi-niveaux (Max>1, ex. Coup Puissant SB+niveau, ou Robuste/`RULES-T0131` +30% Blessures/niveau) pris a un cran de carriere >=2 recevait directement 2/3/4 rangs d'un coup. **Gunther a aussi Robuste (`RULES-T0131`) fige a "4" rangs dans `MetierTalent`** (cran de carriere niveau 4, jamais augmente depuis) - meme bug, impact sur le total de Blessures a verifier par Nono une fois le personnage rouvert. Corrige : `PersonnageTalent.Valeur := 1` (un choix de carriere accorde toujours exactement un rang, les rangs suivants ne peuvent venir que de l'Augmentation). **XML de Gunther deja corrompu, corrige a la main** (comme le cas `CurrentXp="0"` du 11/09/2026, une correction de code ne repare pas un fichier deja ecrit) : les 16 entrees de `CHAPTER_TALENT/SUBCHAPTER_TALENTCAREER` du dernier fichier (`20260926-062150.xml`) remises a "1" (Coup Puissant et Robuste inclus). Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes). **A TESTER PAR NONO** : rouvrir Gunther Krieg, verifier Coup Puissant = niveau 2 (1 carriere + 1 augmente), Gaffe a `DR+12`, et le total de Blessures (Robuste passe de 4 a 1 rang) - **peut faire baisser ses Blessures maximales**, a valider que c'est la vraie valeur voulue. **Reste** : d'autres personnages avec un talent multi-niveaux pris a un cran de carriere >=2 peuvent porter la meme donnee corrompue - a corriger au cas par cas si Nono en repere.
