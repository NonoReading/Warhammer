...
**RUNES DE MACHINES : ModifyReloadRate AJOUTE AU MOTEUR, RUNE OF RELOADING CABLEE SUR LES 2 PDF (26/09/2026, compile 0 erreur, A TESTER PAR NONO)** : chantier ouvert le meme jour en discutant de la structure de sauvegarde des personnages (Attributs/Competences/Talents scindes en chapitres XML). Trois Runes de machines du Dwarf Player's Guide (Accuracy/Penetrating/Reloading/Seeking) etaient saisies en `<Craftsmanship>` avec `<Modifier>` vide, donc sans effet. Decisions prises avec Nono : Accuracy reutilisera `BonusAccurate` (deja code pour la qualite d'arme Accurate, pas touche aujourd'hui - reste a generaliser) ; Seeking reste NON MECANISE (bonus conditionne au type de cible visee - volante -, hors moteur, doit rester du texte) ; Reloading MECANISEE aujourd'hui. Nouvelle cible `ModifyReloadRate` au vocabulaire du moteur generique `ChargeModificateur` (meme moule que `ModifBoat`/`ModifyDamage`) : `chargeconstantes.pas` (`ConstXmlModifieRechargement`/`ConstCibleModifieRechargement`, `BonusReload`=`RULES-WEAPB23`, `BonusSalvo`=`UPINA-WEAPB36`), parsing `DATA_CRAFTMANSHIP` branche (`xmlexportimport.pas` ~L3505, meme case que `ModifArmure`/`ModifyDamage`). XML de Rune of Reloading (`DWARF-RUNE_61`) porte desormais `<ModifyReloadRate name="RELOAD">-2</ModifyReloadRate>` (chiffre reel du livre, -2 par rune, pas -1) et `<Applies>` corrige `Misc`->`Weapon` (comme Rune of Cleaving). Nouvelle fonction `ArmeReloadEffectif` (`chargefabrication.pas`, `Base` en parametre `out`) : lit le chiffre de base sur `PArme.ListeBonus` (Qualite Reload N), soustrait la somme des `ModifyReloadRate` des runes portees (`FabricationModificateurQualite`), plancher 1 si la Qualite Salvo est presente sinon 0 (Qualite retiree, rechargement libre), -1 si l'arme n'a pas la Qualite Reload. Cablee dans les DEUX blocs armes de `pdfpersonnage.pas` (`PdfPersonnageCreation` ~L1885 et Feldo2P ~L3672 - meme reflexe que le bug `ModifyWeapon`/Feldo2P du 08/09/2026) : remplace "Reload N" par le chiffre effectif dans `LigneBonus` juste apres `FabricationDetail`, ou retire la Qualite si elle tombe a 0. Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes). **A TESTER PAR NONO** : arme avec Qualite Reload + Rune of Reloading sur les deux PDF, et une arme Salvo pour verifier le plancher a 1. **Point de reprise** : Accuracy (generaliser `BonusAccurate` a `<Craftsmanship>`) et l'ecran WinWeapon/WinPersonnage pour Reload (actuellement seul le PDF est cable, `GetAllArmeBonusLibelle` n'a pas le contexte personnage) restent a faire si Nono le demande - voir `A FAIRE.txt` "RUNES DE MACHINES".
...
**TALENT DE CARRIERE (MetierTalent) COMPTE "NIVEAU DE CARRIERE" RANGS AU LIEU DE 1 DANS LE CALCUL DE DEGATS/EFFETS (26/09/2026, TESTE ET VALIDE PAR NONO. CHANTIER ENTIEREMENT CLOS : la limite decrite plus bas a ete corrigee le meme jour, voir la fin de ce paragraphe.)** : signale par Nono sur Gunther Krieg - la Gaffe (Boat Hook, `RULES-COMB_BASE_02`, degats `SB+4`) affichait `DR+14` au lieu de `DR+12` attendu apres avoir augmente Coup Puissant (`RULES-T0037`) de 1 a 2 sur l'ecran d'Augmentation. **Premiere piste explorée puis abandonnee** : stocker `1` au lieu de `PMetierTalent.NiveauMetier` dans `Personnage.MetierTalent` a l'entree en carriere (`winpersonnage.pas` ~L6379) - revertee, ce champ sert AUSSI de filtre ailleurs (`CalculAvancement` ~L5977 : `PersonnageTalent.Valeur = StrToInt(MetierNvEnCours)`, pour proposer les talents nouvellement debloques au palier atteint) ; l'ecraser a 1 aurait casse cette comparaison des le niveau 2 (plus aucun talent de carriere propose au-dela du niveau 1). **Cause reelle, confirmee par Nono** ("j'ai pris coup puissant par la carriere actuelle puis l'ai augmente au niveau 2") : `Personnage.MetierTalent[].Valeur` porte deliberement le NIVEAU DE CARRIERE ou le talent apparait dans la table (`StructureMetierTalent` n'a pas de champ "nombre de rangs" - `chargemetiertalent.pas`), pas un nombre de rangs - mais `PersonnageTalentModificateur` (`chargepersonnage.pas` ~L1889-1896, point de consommation pour ModifyDamage/ModifyCarac/ModifyWeapon/ModifyArmure) le multipliait quand meme par `Facteur` comme s'il s'agissait d'un rang. Invisible pour un talent a rang unique (Max=1, la plupart), un talent multi-niveaux (Max>1, ex. Coup Puissant SB+niveau) pris a un cran de carriere >=2 comptait directement 2/3/4 rangs, en plus des rangs achetes ensuite via l'Augmentation (`Personnage.AugmentationTalent`, boucle suivante de la meme fonction) sans recoupement. **Corrige au point de consommation** : la boucle `MetierTalent` de `PersonnageTalentModificateur` utilise desormais `Facteur*1` (un pick de carriere accorde toujours exactement un rang), au lieu de `Facteur*PersonnageTalent.Valeur` - le champ `Valeur`/`NiveauMetier` reste intact partout ailleurs (ecriture XML, filtre `CalculAvancement`). **XML de Gunther deja ecrit avec la donnee incoherente** : `CHAPTER_INCREASE/SUBCHAPTER_TALENT` `RULES-T0037` corrige a la main de `"2"` a `"1"` dans le dernier fichier (`20260926-062150.xml`) - avec la carriere qui compte desormais pour 1 rang fixe (au lieu de 2 avant la coche), le total voulu par Nono (2) = 1 (carriere) + 1 (augmentation), pas 1+2. `CHAPTER_TALENT/SUBCHAPTER_TALENTCAREER` laisse INCHANGE (les niveaux 1/2/3/4 d'origine, y compris Coup Puissant a "2" et Robuste a "4" - ce sont des niveaux de carriere, pas des rangs, ne plus les "corriger" a 1). Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes). **A TESTER PAR NONO** : rouvrir Gunther Krieg, Gaffe a `DR+12`. **Limite trouvee le meme jour, corrigee dans la foulee** : l'ecran Augmentation (`TabAugmentationTalent`, colonne "Actuel") lit le total affiche sur `TabTalent` (`ColTalNb`), qui ne fusionnait QUE Creation+Augmentation - `Personnage.MetierTalent` n'y etait jamais merge. Un talent multi-niveaux pris via la carriere restait donc INVISIBLE sur cet ecran. **Corrige** : nouvelle boucle sur `Personnage.MetierTalent` dans `XmlChargePersonnage` (`winpersonnage.pas` ~L3712, juste apres celle de `CreationTalent`), fusion par code avec la ligne existante, ajout dans `ColTalNb` ET `ColTalNbCrea`. Le calcul du delta a la sauvegarde (~L6215, `Nouveau - ColTalNbCrea`) en beneficie sans etre touche - c'etait deja le bon levier, il manquait juste d'y integrer `MetierTalent` au chargement. Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes), teste et valide par Nono. **Reste** : d'autres personnages avec un talent multi-niveaux pris via la carriere ET augmente auparavant peuvent avoir un XML deja errone (meme correction manuelle que Gunther Krieg si Nono en repere un).

**OPTION "DIVISER L'XP PAR 25" : COCHER LA CASE NE CONVERTISSAIT PAS LE TOTAL, ET LE XML SAUVEGARDAIT LE TOTAL DIVISE DEUX FOIS (26/09/2026, TESTE ET VALIDE PAR NONO. CHANTIER CLOS.)** : signale par Nono sur Gunther Krieg (comparaison des deux derniers XML de sauvegarde, `SAVED_CARACTERS/WFRP4/Gunther Krieg/20260926-060814.xml` avant coche / `...060827.xml` apres coche). **Bug 1** : cocher `CheckBoxXpDiv25` sans avoir tape dans `EditTotalXp25` sauvegardait `Xp="0"`/`Xp25="0"`/`CurrentXp="20"` au lieu d'une conversion coherente de `Xp="2675"`/`CurrentXp="175"`. Cause : `CalculXpGlobal` (`winpersonnage.pas`, appelee par `CheckBoxXpDiv25Change`) etait un stub totalement vide - rien ne recalculait `EditTotalXp25` depuis `EditTotalXp` a la coche (seule la frappe manuelle dans l'un des deux champs les synchronisait, via `EditTotalXpKeyUp`/`EditTotalXp25KeyUp`). Corrige : `CalculXpGlobal(Mode25: boolean)` alimente desormais `EditTotalXp25.Text` par `Trunc(EditTotalXp/25)` quand `Mode25=true`, meme conversion que `EditTotalXpKeyUp` ; rafraichit aussi la ligne "Total" de `tabExperience` (restait figee sur l'ancienne valeur brute a l'ecran, `LigXpTotal` n'etant sinon repose qu'au chargement) et appelle `CalculTableExperience()` pour que Depense/Restant suivent immediatement. **Bug 2, trouve en relisant le XML apres le fix du bug 1** : une fois `EditTotalXp25` correctement alimente (107), le fichier sauvegarde portait quand meme `Xp="107"`/`Xp25="4"`/`CurrentXp="127"` au lieu de `Xp="2675"`/`Xp25="107"`/`CurrentXp="127"` - `<Xp>` doit toujours rester le total BRUT (c'est ce que `PersonnageXmlCreation`, `chargepersonnage.pas`, redivise elle-meme par 25 pour ecrire `<Xp25>`), mais `XmlSauvegarde` (`winpersonnage.pas`) lui passait `CalculXpEtat().Total`, qui bascule deja en unite divisee des que la case est cochee (necessaire pour que le Restant affiche a l'ecran soit juste) - double division en cascade. Corrige : le parametre `Xp` de l'appel `PersonnageXmlCreation` lit desormais `EditTotalXp.Text` (toujours brut), seul `XpRestante` (`CurrentXp`) reste sur l'unite courante, comme deja documente le 11/09/2026 pour ce meme champ (voir plus bas dans ce fichier, entree "PDF FELDO"). Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes) et **teste et valide par Nono** : Gunther Krieg, coche cochee, `Xp="2675"`/`Xp25="107"`/`CurrentXp="127"` corrects dans le nouveau XML.

**BUG DE SAUVEGARDE DES AUGMENTATIONS DE COMPETENCE (25/09/2026, TESTE ET VALIDE PAR NONO. CHANTIER CLOS.)** : signale par Nono, `MajTables` (`winpersonnage.pas`, ~L6133) ajoutait a tort la valeur `Actuel` de `TabAugmentationCompetence` au total sauvegarde (`TabCompetence.Cells[ColCompWork,...]`) au lieu de la difference reellement achetee (`Nouveau - Actuel`) - une augmentation de 15 a 20 sauvegardait +15 au lieu de +5, gonflant le total a chaque augmentation ulterieure. Corrige en ajoutant `Nouveau - Actuel` plutot que `Actuel` seul. Teste et valide par Nono.

**WINFABRICATION, THEME TRADUIT EN FRANCAIS (25/09/2026, TESTE ET VALIDE PAR NONO. CHANTIER CLOS.)** : la colonne Theme et le menu deroulant `CombTheme` de l'ecran ci-dessous affichaient le code brut (`ARMOB`/`DEFECT`/`QUALITY`/`RUNE`/`WEAPB`) au lieu d'un libelle traduit. Nouvelle fonction `FabricationThemeLibelle` (`chargefabrication.pas`), qui mappe ces 5 codes (seuls themes existants a ce jour dans les `DATA_CRAFTMANSHIP` du projet) vers `RULES-LAB_290` a `294` (nouvelles entrees `INTERFACE.Xml`/`INTERFACE_FRANCAIS.Xml` : Armour Bonus/Defect/Quality/Rune/Weapon Bonus). `winfabrication.pas` utilise ce libelle pour l'affichage (colonne 7, `CombTheme`) ; le tri interne (cle `"theme|libelle"`) garde le code brut `FabricationTheme`, inchange. Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes) et teste a l'ecran par Nono (colonne et menu en francais). **Reste** : un theme non prevu dans un futur livre retombe sur le code brut non traduit (`else` du `case`) - ajouter son `RULES-LAB_*` a ce moment-la.

**ECRAN DE CHOIX DES FABRICATIONS REGROUPE PAR THEME, AVEC FILTRE DEROULANT (25/09/2026, compile 0 erreur, A TESTER PAR NONO)** : Nono remarque que l'ecran WinFabrication melange a plat toutes les fabrications de tous les livres actifs (Lightweight/Fine/Durable du Rulebook, 71 runes naines...), rendant la liste illisible des que plusieurs livres sont actifs - le but demande est de "trouver plus facilement". Decision prise avec Nono : deduire le theme d'une fabrication de son CODE (`LIVRE-THEME_suffixe`, ex. `DWARF-RUNE_01` -> theme `RUNE`, `RULES-QUALITY_01` -> `QUALITY`, `RULES-DEFECT_01` -> `DEFECT`) plutot qu'un nouveau champ XML a saisir a la main sur les 71 runes existantes - convention deja respectee par toutes les entrees `DATA_CRAFTMANSHIP` du projet (Rulebook, Dwarf Player's Guide, Nations of Mankind, Horned Rat Companion). Nouvelle fonction `FabricationTheme(CodeFabrication)` (`chargefabrication.pas`) : tout ce qui suit le dernier `_` du code, apres le `SeparateurLivre`. `winfabrication.pas` (`FormCreate`) : nouvelle colonne "Theme" (`RULES-LAB_289`, ajoutee a `INTERFACE.Xml`/`INTERFACE_FRANCAIS.Xml`) en fin de grille (colonne 7, les indices existants 1/3/4/5/6 inchanges) ; la liste filtree est desormais triee par cle "theme|libelle" (`TStringList.Sorted`, l'index d'origine dans `ListFabrication` porte comme objet associe pour retrouver l'entree sans collision entre deux lignes de meme theme/libelle) avant remplissage de la grille, au lieu de l'ordre de chargement des livres. **Suite demandee par Nono le meme jour** : liste deroulante `CombTheme` (`.lfm`, meme moule que `CombTheme`/`ChargeThemes` de `winequipement.pas`), peuplee des themes presents (precedee de "Tous", `RULES-LAB_200` deja existant) - `FiltreTheme` cache les lignes hors filtre par `RowHeights[I] := 0` (meme ruse que `ColWidths[1] := 0` pour la colonne Code cachee, deja dans ce fichier) plutot que de reconstruire la grille, pour ne jamais perdre une quantite deja saisie par Nono sur une ligne qui sort temporairement du filtre. Voir aussi `CONTEXT.md` §1.1 (index des mecanismes "objet special" par theme, ajoute plus tot dans la meme session). Compile (`lazbuild --build-all`, 0 erreur, meme profil warnings/hints/notes, +1 hint "Sender not used" attendu sur le nouveau `CombThemeChange`). **Reste a Nono** : ouvrir WinFabrication sur un objet d'un personnage avec plusieurs livres actifs (Rulebook + Dwarf Player's Guide par exemple), verifier la colonne Theme, le filtre deroulant, et qu'une quantite saisie sur une ligne survit a un changement de filtre puis a "Tous".

**MARQUEUR (Q) "QUALITE A CHOISIR" TRADUIT EN FRANCAIS (25/09/2026, VALIDE PAR NONO. CHANTIER CLOS.)** : dernier point ouvert de `A FAIRE.txt` "VARIANTES (Q)". Decision de Nono avant de coder : le marqueur `RULES-QUALITY_AJOUT` (pose quand un objet de carriere note `(Q)` est acquis, cf. `QualiteDepuisCode`/`QualiteSansMarqueur`, chargefabrication.pas) reste volontairement DIFFERE - pas de popup de choix de qualite force a la creation ni a l acquisition : c'est au MJ de decider quand le joueur doit trancher (il peut acheter une arme sans pouvoir encore choisir sa qualite). L'idee "choix immediat parmi DATA_CRAFTMANSHIP" du `A FAIRE.txt` (chantier 2.77 suite 4) est donc **tranchee : ne pas la faire**, ne plus la ressortir en anomalie. Seul reste corrige : ce marqueur n'avait AUCUNE entree dans `BOOK_RULESBOOK_FRANCAIS.Xml` (WFRP4) - s'affichait donc en anglais brut ("Quality to add") dans la colonne Fabrication de WinPersonnage, meme en interface francaise. Entree `<Craftsmanship id="RULES-QUALITY_AJOUT">` ajoutee (Description "Qualite a choisir", Explanation et Short traduits), memes balises que ses voisines `RULES-DEFECT_0x`. WFRP5 n'a pas de fichier francais (comme le reste du programme, aucune traduction sur les autres livres) : rien a faire de ce cote. XML seul, aucun code Pascal touche. Compile (`lazbuild --build-all`, 0 erreur) et valide par Nono le 25/09/2026 (affichage "Qualite a choisir" sur la fiche). **Chantier "VARIANTES (Q)" entierement clos.**

**SCISSION DE L'EQUIPEMENT PAR CATEGORIE, ETAPE 4 - CATALOGUE +EQUIPMENT FILTRE PAR CATEGORIE (25/09/2026, TESTE ET VALIDE PAR NONO. CHANTIER CLOS.)** : Nono releve que `+Equipment` (bouton `ButtonEquipement`, catalogue `DATA_TRAPPING`) affichait tout le catalogue (montures/bateaux/vehicules ET divers melanges) quelle que soit la categorie choisie dans `ListBoxEquipementCategorie` - genant sur la categorie Montures/Navires/Vehicules, noyee dans tous les Divers. Mecanisme deja existant reutilise (verifie avant de coder, cf. regle du projet) : `TModeEquipements`/`ModeEquipements` (`winequipement.pas`), deja utilise par `TMenu.TabBibliothequeDblClick`/`ButtonEquipementClick` (`warhammersource.pas`) pour la meme distinction depuis le menu principal - aucun nouveau code de filtrage, juste le meme reglage pose avant `TWinEquipements.Create` dans `winpersonnage.pas` (`ButtonEquipementClick`) : `meVehicules` (theme `RULES-LAB_195` seul) si `ListBoxEquipementCategorie.ItemIndex = 0`, sinon `meHorsVehicules` (tout sauf ce theme, categorie Divers). Compile (`lazbuild --build-all`, 0 erreur) et teste a l'ecran par Nono le 25/09/2026 sur les deux categories. **Chantier "scission de l'equipement par categorie" (etapes 1 a 4) entierement clos.**

**SCISSION DE L'EQUIPEMENT PAR CATEGORIE, ETAPE 3 - BOUTONS CONTEXTUELS ET LARGEUR DES COLONNES (25/09/2026, TESTE ET VALIDE PAR NONO. CHANTIER CLOS.)** : deux finitions demandees par Nono apres l'etape 2 (ci-dessous). (1) **Boutons contextuels** : `ButtonArme`/`ButtonArmure`/`ButtonEquipement`/`ButtonSort` (ajout) et `ButtonFabrication`/`ButtonDelete` (action sur la ligne selectionnee) ne s'affichent desormais que s'ils ont un sens dans la categorie choisie dans `ListBoxEquipementCategorie` (`ButtonEquipement`, qui ouvre le catalogue `DATA_TRAPPING`, sert aux deux categories Montures/Navires/Vehicules ET Divers, qui s'ajoutent par ce meme bouton) - logique posee dans `RafraichitTabEquipementAffichage` (seul point de passage a chaque changement de categorie ET a chaque ajout/suppression). **Piege trouve en testant l'ouverture de la fenetre** : ce calcul etait bien fait a l'ouverture, mais un bloc plus loin dans la meme fonction de chargement (`winpersonnage.pas`, ~L3985, affichage d'un instantane d'historique en lecture seule) reecrivait `Visible` en bloc (`true`/`false` sur les 4 boutons d'ajout, sans notion de categorie) - remplace par un nouveau flag global `EquipementModifiable` (unit-level, comme `FabAjoute`), croise avec le filtre de categorie dans `RafraichitTabEquipementAffichage`. (2) **Largeur des colonnes** : `TabEquipementAffichage` (grille visible) copiait telles quelles les largeurs de `TabEquipement` (grille masquee, toujours complete) - or `AdjustGridColumnsWidth` n'est appelee que sur `TabEquipement`, a chaque ajout d'objet, et dimensionne les colonnes pour TOUT l'inventaire, pas pour la categorie filtree affichee : les colonnes visibles restaient figees sur le dernier calcul, sans rapport avec le contenu reellement affiche. Corrige par un appel direct a `TabEquipementAffichage.AutoSizeColumn(Col)` sur chaque colonne visible (largeur non nulle), apres remplissage des lignes filtrees, dans `RafraichitTabEquipementAffichage` - `AutoSizeColumn` ne touche que la largeur de la colonne, jamais celle de la grille (qui reste geree par l'ancrage proportionnel, `ancrageproportionnel.pas` : lui toucher aurait pu casser la reference prise au premier `Appliquer`). Compile (`lazbuild --build-all`, 0 erreur, memes warnings/hints) et teste a l'ecran par Nono le 25/09/2026 (boutons corrects par categorie, y compris a l'ouverture et sur un instantane d'historique ; colonnes ajustees au changement de categorie). **Chantier "scission de l'equipement par categorie" entierement clos.**

**SCISSION DE L'EQUIPEMENT PAR CATEGORIE (24-25/09/2026, ETAPES 1 ET 2 FAITES ET VALIDEES PAR NONO. CHANTIER CLOS.)** : nait de la remarque de Nono en voulant restreindre les amenagements de bateau (ci-dessous) a la bonne taille de navire. **Etape 1** (24/09/2026) : `PageExperience` (10 onglets Attribut/Competence/Talent/XP/Evolution/Historique/Livre/MjCost/Corruption/Mutation, "cela devient complique" selon Nono) convertie de `TPageControl` a `TNotebook` pilote par une `TListBox` laterale (`ListBoxExperience`, `winpersonnage.pas`/`.lfm`) - la liste choisit la page affichee, meme role que la bande d'onglets avant. `TNotebook` n'a ni `ShowTabs` ni `OnPageChanged` publies dans la LCL installee (verifie dans `C:\lazarus\lcl\extctrls.pp`) : proprietes retirees du `.lfm`, `Caption`/`ClientHeight`/`ClientWidth` des pages aussi (geres par ailleurs, Caption reaffecte par code dans `AfficheImageRace`). Symptome bloquant en cours de mise au point : texte des controles enfants (ex. `RadioButtonRAS`/`Suivant`/`Changer`) invisible sur fond noir - deux pistes fausses explorees (repaint force `Invalidate`, cablage direct de l'evenement de changement de page), la vraie cause etait juste la couleur de fond de `ListBoxExperience` a regler explicitement (`Color`, proche de `clNone`, resolu par Nono via l'IDE). **Lecon** : verifier la couleur avant de partir sur des pistes d'evenements/repaint. **Etape 2** (25/09/2026, testee et validee par Nono a l'ecran) : meme principe applique a `TabEquipement`, mais SANS reprendre le mecanisme `TNotebook` — `TabEquipement` etait deja lu/ecrit par des dizaines de procedures (`TabEquipement.Row`/`.Cells` : ajout, suppression, edition Fabrication/Quantite, popup "Confier a...", double-clic Fiche, et surtout les deux boucles de synchronisation vers `Personnage.Equipement`, `CalculTotaux` et `MajTables`, cf. audit par sous-agent avant de coder). Architecture retenue : `TabEquipement` reste la grille UNIQUE de sauvegarde, toujours complete, mais **masquee** (`Visible := False`, .lfm) — aucune de ces procedures n'a change. Deux nouveaux controles visibles, memes coordonnees que l'ancien `TabEquipement` (`AjustePositionTables`) : `ListBoxEquipementCategorie` (5 entrees : Montures/Navires/Vehicules `RULES-LAB_286` nouveau, puis Arme/Armure/Divers/Sort reutilisant `RULES-LAB_063/065/064/083` existants) et `TabEquipementAffichage` (grille filtree, seule visible a l'ecran). Nouvelle fonction `CategorieEquipementLigne(Ligne)` (0 a 4) : un bateau/vehicule garde le type Divers en colonne 3 (pas de type dedie) - `LigneEstAnimal` (profil animal/bateau/vehicule via `TrappingEstPorteur`) est donc teste EN PREMIER, avant le test par colonne Type ; un sort n'a pas de TypeEquip fixe (`TypeSort` est un libelle libre du livre) - identifie par `TalentSort(Code).CodeTalent <> ''`, meme test que `ButtonFabricationClick`/`TabEquipementSelectCell` deja existants. `RafraichitTabEquipementAffichage()` reconstruit `TabEquipementAffichage` depuis `TabEquipement` a chaque changement de categorie et apres toute modification (appelee dans `RafraichitPorteurs` - point de sync commun a Confier a/Porte/Reprendre/suppression/chargement - et explicitement apres chaque ajout Arme/Armure/Divers/Sort/Fabrication/carriere). Interaction sur la grille filtree -> ligne reelle : `LigneEquipementReelle(Ligne)` retrouve la ligne de `TabEquipement` par le Code (colonne 2, identique dans les deux grilles), utilisee par `TabEquipementMouseDown`/`TabEquipementSelectCell` pour maintenir `TabEquipement.Row` synchronise avant tout bouton/menu qui agit dessus (`ButtonDelete`/`ButtonFabrication`/`MenuConfier`/`MenuPorte`/`MenuReprendre`, tous inchanges). Edition en place de la Quantite (seule colonne editable, `TabEquipementSelectEditor`) propagee vers `TabEquipement` par `TabEquipementAffichageSetEditText` (`OnSetEditText`). `TabEquipementPopup`/`TabEquipementDblClick`/`TabDrawCell` rebranches sur `TabEquipementAffichage` sans changer leur corps (ils lisent deja `TabEquipement.Row`/`.Cells`, ou sont generiques via `Grid := TStringGrid(Sender)`). Compile (`lazbuild --build-all`, 0 erreur) et **teste a l'ecran par Nono le 25/09/2026** : les 5 categories filtrent bien, "Confier a..."/Fiche/suppression/ajout fonctionnent entre grille filtree et grille de sauvegarde. **Chantier clos.**

**AMENAGEMENTS DE BATEAU, CALCUL DE L'EFFET SUR LE PROFIL (24/09/2026, compile 0 erreur, les 4 deltas codes - Fore-and-Aft Rudder, Smoothing, Armoured, Sturdy - TESTES ET VALIDES PAR NONO. CHANTIER CLOS.)** : suite du chantier rouvert le 23/09 (voir Log.txt). Audit par sous-agent des deux livres (Death on the Reik Companion + Sea of Claws) : sur les 76 entrees + traits deja catalogues (Armoured/Sturdy/Ram/Rugged/Unwieldy...), **seuls 7 sont un vrai delta chiffrable de profil** (Crew/M/Man/Size/T/W) - tout le reste (Broad Rudder, Armoured Walls, Gun Ports, Luxury Cabins, Raised Gunwales, Stripped, Flying Jib, Racing Rig, Closed Rowlocks, Spoons, Musket Rests, Anchor, Armour Plating, Crow's Nest, Figurehead, Ram, Rugged, Unwieldy) donne des bonus/malus de test (SL), de la couverture au tir ou de l'AP de coque (pas un +T) - **hors perimetre par decision de Nono (24/09/2026)**, meme logique que la decision du 20/09 (regles de conduite/equipage = MJ). Decision de Nono : commencer par les deltas de profil chiffrables, voir ensuite pour le reste. Sur les 7 releves, **4 codes** (les 3 autres laisses de cote, voir A FAIRE.txt) : Fore-and-Aft Rudder (`DEATH-TRAP_26` a `_29`, M -1 fixe), Smoothing (`SEAOF-TRAP_052`/`_053`, M +1 voile ET rames - vendu "par 5 yards de Size" mais l'effet ne se cumule pas avec la quantite achetee), Armoured (trait deja au catalogue, ex. `SEAOF-TRAP_020+` `AnimalTraits` "Armoured 2" : +10 Toughness par niveau) et Sturdy (idem, +30% Wounds par niveau). Laisses de cote : Water Brakes (M -1/-2 seulement TEMPORAIREMENT a l'usage, pas un delta permanent) et Steam/Steam Engine (incoherent entre les deux livres : DotR "maintient le M standard" sans vent a simuler donc aucun delta, Sea of Claws M fixe absolu a 4 donc pas additif). **Implementation, mecanisme existant reutilise** (verifie avant de coder, cf. regle du projet) : nouveau `TypeModif` `ModifBoat` (chargeconstantes.pas), **meme moule que `ModifArmour`/`ModifyCarac`** du moteur generique `ChargeModificateur` (deja utilise pour les qualites de fabrication sur arme/armure) - parse sur `DATA_TRAPPING` (xmlexportimport.pas, meme bloc que `<ModifArmour>` sur `DATA_CRAFTMANSHIP`), somme via `FabricationModificateurQualite` (chargefabrication.pas) **deja existant, sans aucune modification** : aucun nouveau moteur de calcul, juste une nouvelle cible generique "M" (appliquee a la fois a M voile et M rames, le livre ne precisant pas laquelle - simplifie depuis "MSAIL"/"MOAR" separees) branchee sur le moteur existant. Nouvelle fonction `BoatProfilCalcule` (chargetrapping.pas, nouvelle dependance vers ChargeFabrication, verifie sans cycle) : part du `ProfilBateau` de base (7 valeurs ';'), ajoute Armoured/Sturdy lus dans `TraitsAnimal` (nouvelle fonction `NiveauTraitBateau`, meme decoupage que `DescriptionTraitsBateau` de winanimal.pas), ajoute les `ModifBoat` des amenagements confies au bateau. Nouvelle fonction `CodesFittingGrille` (winpersonnage.pas), calquee sur `ChargeAnimalGrille` (meme colonne 11 `PortePar`/colonne 10 Id) - **niveau force a 1** (present/absent) quelle que soit la quantite reellement achetee dans la grille : necessaire pour Smoothing, qui se vend par tranche de 5 yards de Size (plusieurs unites pour couvrir un grand bateau) sans que l'effet M+1 ne se cumule avec le nombre d'unites. `winanimal.pas` (`AfficheFicheAnimal`) affiche desormais `BoatProfilCalcule(...)` au lieu du `ProfilBateau` brut sur la fiche bateau (bouton Fiche de WinEquipement - catalogue seul, sans amenagement -, double-clic de WinPersonnage - avec les amenagements confies). Compile (`lazbuild --build-all`, 0 erreur, aucun warning nouveau). **BUG TROUVE ET CORRIGE EN TESTANT (24/09/2026)** : les colonnes du profil de bateau ne sont PAS de simples entiers ("12 (30)", "-", "4 (vapeur)"...) - `StrToIntDef` transformait a tort un "-" (bateau sans voile, ex. Fishing Skiff) en 0, donnant un M errone apres le malus du Fore-and-Aft Rudder. Corrige par deux nouvelles fonctions `AjusteValeurEnTete`/`AjusteValeurProportionnelle` (chargetrapping.pas) qui n'ajustent que le nombre en tete de la colonne et preservent tout suffixe, sans toucher aux colonnes non-numeriques. **Fore-and-Aft Rudder TESTE ET VALIDE PAR NONO** (Fishing Skiff, M Sail "4(-)"/M Oar "6(8)" -> "3(-)"/"5(8)" apres attache). **Smoothing et Armoured/Sturdy TESTES ET VALIDES PAR NONO le 24/09/2026** (bateau du catalogue portant les traits, M/T/W calcules corrects sur la fiche). Les 4 deltas codes sont donc tous testes. Water Brakes et Steam/Steam Engine restent hors moteur (decision de Nono, effet non chiffrable en delta fixe - voir plus haut). **Chantier clos.** Voir Log.txt et A FAIRE.txt.

**CLASS TRAPPINGS (23/09/2026, compile et valide par Nono)** : dernier ecart du chantier "AUDIT DES TXT" sur le Rulebook (chapitre II, p.37). Equipement de depart par Classe (8 classes : Academics/Burghers/Courtiers/Peasants/Rangers/Riverfolk/Rogues/Warriors), distinct de l'equipement par Carriere deja saisi (chapitre III), donne en plus a la creation. Le champ `<Class>` existait deja sur chaque carriere (charge dans `StructureMetier.LibelleGroupe`) mais n'etait lu par aucun .pas. Nouvelle unite `chargeclassequipement.pas` (`StructureClassEquipement`/`ListClassEquipement`, memes champs que `StructureMetierEquipement` moins le niveau, pour reutiliser telle quelle la logique d'affichage). XML : nouvelle balise `<DATA_CLASS_ITEM>` dans `BOOK_RULESBOOK.Xml` (EN seul, pas de texte a traduire - que des codes catalogue ou des noms libres comme `BOOK_WINDS_OF_MAGIC.Xml` le fait deja pour "Fine Clothing"), meme format `SUBCHAPTER_ITEM`/`Item` que l'equipement de carriere, memes mecanismes `+` (ensemble, ex. Flask+Spirits pour Riverfolk) et `quantite=` (Candles x2 pour Rogues) deja existants, zero nouveau mecanisme XML. Codes catalogue retrouves pour la plupart des objets (Clothing `RULES-TRAP_017`, Dagger `RULES-COMB_BASE_10`, Pouch `RULES-TRAP_007`, Sling Bag `RULES-TRAP_011`, etc.) ; objets sans code catalogue laisses en texte libre (Fine Clothing, Lunch, Parchment 1d10 sheets, Matches 1d10) - meme convention que l'existant. Cablage dans `wincreation.pas` (`AfficheImageMetier`) : apres la boucle d'equipement de Carriere (niveau 1), une seconde boucle sur `ListClassEquipement` filtree par la Classe de la carriere en cours (`ChercheMetier(MetierEnCours).LibelleGroupe`), ajoutee aux memes lignes de `TabMetierEquipement` - code deliberement duplique (pas de fonction commune) pour rester au plus pres du pattern existant plutot que refactorer une logique de branchement Arme/Armure/Divers deja complexe. Compile (lazbuild, 0 erreur) et teste par Nono le 23/09/2026 (Villager/Peasants, objets de classe bien ajoutes en plus de la carriere). **Chantier "AUDIT DES TXT" du Rulebook desormais clos entierement** (I-II/III/IV/V/VI/VII/VIII/IX/XI tous audites, tous les ecarts trouves sont saisis).

**CATALOGUE DES MALADIES (23/09/2026, compile et valide par Nono)** : suite de l'audit du Rulebook (chapitre V, ecart trouve le 23/09). 9 maladies de "The Litany of Pestilence" (p.186-188) saisies dans `BOOK_RULESBOOK.Xml`/`_FRANCAIS` : `RULES-DISEASE_001` a `_009` (Blood Rot, The Bloody Flux, The Black Plague, Festering Wound, Galloping Trots, Itching Pox, Minor Infection, Packer's Pox, Ratte Fever), balise `<DATA_DISEASE>`, chaque entree avec Description/Contraction/Incubation/Duration/Symptoms en texte libre (pas d'effet mecanise, comme le catalogue de mutations qui a servi de modele). Association Contraction/Incubation/Duration a chaque maladie retrouvee en rendant les pages 186-188 en image (poppler `pdftoppm`) pour lever l'ambiguite de la mise en page a 2 colonnes du fichier texte source (le simple export texte melangeait les blocs de deux maladies adjacentes). Nouvelle unite `chargedisease.pas` (StructureDisease/ListDisease, calquee sur `chargecorruptiontable.pas` mais sans modificateur ni table de chance D100). Fenetre de consultation `windiseasecatalogue.pas` (sans .lfm, calquee sur `winmutationcatalogue.pas`), ouverte depuis la 7e ligne de `TabBibliotheque` (menu principal, `RemplirTabBibliotheque`/`TabBibliothequeDblClick` dans `warhammersource.pas`). Traduction FR geree comme les autres catalogues : `StructureTraduction` (`chargetraduction.pas`) gagne 3 champs (Contraction/Incubation/Duration ; Symptoms reutilise le champ Description existant), nouveau cas `ConstPDisease` dans `Traduit`. Labels `RULES-LAB_281` a `_285` (Diseases/Contraction/Incubation/Duration/Symptoms) ajoutes aux deux INTERFACE. Compile (lazbuild, 0 erreur) et teste par Nono le 23/09/2026 (fenetre catalogue, 9 lignes, compteur a jour). Chantier "AUDIT DES TXT" du Rulebook : reste seulement l'ecart "Class Trappings" (p.37, equipement de depart par Classe) a saisir - voir A FAIRE.txt.

**RULEBOOK, AUDIT COMPLET (23/09/2026, XML seul, compile et valide par Nono)** : audit du livre de base (~2600 identifiants EN) fait par 5 sous-agents en parallele, un par chapitre riche en catalogue : Class and Careers (III, 64 carrieres), Skills and Talents (IV), Religion and Belief (VII, Blessings/Miracles des 10 cultes), Magic (VIII, 8 lores + Witch/Dark/Chaos Magic), The Consumers' Guide (XI, armes/armures/trappings). RAS partout sauf UN ecart : la competence groupee Sail n'avait aucune specialisation saisie (`RULEBOOK.txt` p.129 : Barge, Caravel, Cog, Frigate, Wolfship). **Corrige en 3 temps** (les 2 premiers etaient incomplets, remarques de Nono) : (1) ajout des 5 specialisations `RULES-COMPVOILE_BARGE/_CARAVEL/_COG/_FREGATE/_WOLFSHIP` en `DATA_SKILL_SPECIALIZATION` ; (2) **le skill de base a du etre renomme `RULES-COMPVOILE` -> `RULES-COMPVOILE_*`** (Description "Sail" -> "Sail (Any)", FR "Voile" -> "Voile (au choix)") et TOUTES ses references corrigees dans `BOOK_RULESBOOK.Xml`/`_FRANCAIS` (17 EN, 9 FR : carrieres, ModifySkill, Career.Skill) - raison mecanique : `wincompetence.pas` l.287 n'affiche la table de specialites (TabSpe) que si le code du skill selectionne contient `_*` (`Pos(ValeurGenerique, AffCode.Text)`) ; sans ce suffixe, "Sail" apparaissait comme une competence normale sans indication qu'il faut choisir une specialite, exactement comme Ride (`RULES-COMPCHEV_*`) ou Language (`RULES-COMPLANG_*`) qui n'ont eux non plus aucune variante "bare" sans `_*`. Le champ `<Generique>` des 5 specialisations pointe donc sur `RULES-COMPVOILE_*` (§2.68, lien explicite, n'a pas besoin du suffixe en theorie mais on suit la convention etablie). Codes PDF `"COMPVOILE"` (3 occurrences) laisses tels quels (champ de formulaire PDF, sans rapport avec l id catalogue). **(3) `RULES-COMPVOILE` etait aussi reference par 8 AUTRES livres** (Nono a repere l'oubli) : `BOOK LUSTRIA.xml`, `BOOK_DWARF_PLAYERS_GUIDE.Xml`, `BOOK_HIGH_ELF_PLAYERS_GUIDE.Xml`, `BOOK_LORDS_OF_NAGGAROTH.Xml`, `BOOK_MIDDENHEIM_CITY_OF_THE_WHITE_WOLF.Xml`, `BOOK_NATIONS_OF_MANKIND.Xml`, `BOOK_SALZENMUND_CITY_OF_SALT_AND_SILVER.Xml`, `BOOK_SEA_OF_CLAWS.Xml` (equipement de carriere "Skill", tous corriges en `RULES-COMPVOILE_*`). `DATABASE/WFRP5/BOOK_RULESBOOK.Xml` a aussi un `RULES-COMPVOILE`, mais c'est une base independante (V5, hors perimetre) : non touche. Teste dans WinCompetence (double-clic sur Sail -> les 5 specialites apparaissent dans TabSpe) et valide par Nono le 23/09/2026. **AUDIT ENTIEREMENT CLOS LE 23/09/2026** (chapitres I-IX audites : I-II RAS, V trouve maladies completees, VI-IX hors-perimetre). Glorious Reikland (X) et Bestiary (XII) sont largement hors-perimetre (lore/PNJ/creatures adverses, meme decision que les autres livres), sauf les Creature Traits p.338-341 deja suivis a part (A FAIRE.txt).

**GREEN IZ BEST, ARMES ORQUES (23/09/2026, XML seul, compile et valide par Nono)** : dernier ecart connu du chantier "AUDIT DES TXT" sur ce livre (le bug de Blessures du 22/09 etait deja corrige). 3 armes de Greenskin Equipment (p.352, source `LIVRES/Green_iz_Best.txt` lignes 352-382) : `GREEN-COMB_BASE_01` Choppa (+SB+4, Damaging/Tiring/Orcish*), `GREEN-COMB_BASE_02` Hobgoblin Scimitar (+SB+4, Impale), `GREEN-COMB_HAST_01` Squig Prod (polearm, +SB+3, Undamaging/Prod**). Deux qualites custom creees (memes numeros `GREEN-WEAPB01`/`_02` que le principe HORNE-WEAPB44+ : prefixe de livre, pas de collision possible) : Orcish (perd tout si porte par un non-Orc, gagne Slow) et Prod (+2 SL Animal Care/Animal Training). Reach "Average" -> `RULES-WEAPR_MID`, Scarce -> `RULES-DISPO_LIMITED`, prix "x/" -> PA. Teste dans WinWeapon (filtre par livre) et valide par Nono le 23/09/2026. Le chantier "AUDIT DES TXT" n'a plus que le Rulesbook (FR+EN) a auditer.

**DEATH ON THE REIK COMPANION ET ARMURERIE SKAVEN DU HORNED RAT COMPANION (23/09/2026, XML seul, teste et valide par Nono)** : suite du chantier "AUDIT DES TXT". (1) `BOOK_DEATH_ON_THE_REIK_COMPANION.Xml` : 12 sorts du Lore of Tzeentch (p.98-100) - 3 Petty `DEATH-PETTY_01` a `_03` (Talent `RULES-T0089`, `Minor Magic`) et 9 sorts de lore `DEATH-TZEEN_01` a `_09` (Talent `RULES-T0088_TZEENTCH`, celui que donne la carriere Warrior of Tzeentch, `Domain Magic` comme tout sort propre a une lore) ; "Bension of Tzeentch" garde tel quel (erratum). Et 8 Boat-Mounted Weapons (p.68) `DEATH-PROJ_ING_01` a `_08`, competence `RULES-COMPPROJ_ING` (le livre : "Engineering specialisation"), Blackpowder pose sur les 4 armes a poudre d apres la note du livre, pas de Crewed (absent de la table). Livre clos. (2) `BOOK THE HORNED RAT COMPANION.Xml`, lot 1 = Skaven Armoury (p.32-40) : talent Tail Fighting `HORNE-T0196`, 22 armes + 6 munitions (`HORNE-COMB_*`, `_ENTRA_*`, `_PROJ_*`, `_MUNI_*`), 3 qualites `HORNE-WEAPB44` ZZAP!, `_45` Warpstone, `_46` Timer, 5 fabrications `HORNE-QUALITY_01` a `_05` (Skaven Blades Warpforged/Weeping/Nurglitch/Warp-blade + Warlock Optics, posees par +Qualite), 9 objets `HORNE-TRAP_01` a `_09`. Decisions Nono du 23/09 : pas de prix, disponibilite `RULES-DISPO_EXOTIC` et restriction de clan en commentaire ; les Skaven Blades sont des fabrications ; Blade of Corruption = arme a part (Unique). Ecartes : armes lourdes p.37 (aucune caracteristique). **LOT 2 CLOS LE 23/09/2026** : Skaven Magic (p.41-49) saisi et valide - 4 sorts Petty Magic + Lore of Ruin (13), Plague (10), Stealth (9), 36 sorts au total sur 3 nouveaux talents generiques (HORNE-T0197/198/199, meme convention que RULES-T0172_TZEENTCH). Creatures adverses et de montagne ecartes (meme decision Frostfiend/Djinn). Livre Horned Rat Companion entierement clos.

**ENEMY IN SHADOWS COMPANION, SORTS DE TZEENTCH (22/09/2026, XML seul, teste et valide par Nono)** : chapitre 9 "The Purple Hand" (p.78-85). 23 sorts ajoutes a `BOOK ENEMY IN SHADOWS COMPANION.Xml` (nouvelle `DATA_SPELL`) : 10 Chaos Arcane Spells `ENEMY-CHAOS_01` a `_10` (Blast/Bolt of Corruption, Daemonic Mien, Foul Messenger, Joyous Aspect, Obsession, Power of Chaos, Rend Aethyr, Slave to Darkness, The Purple Hand - Talent `RULES-T0172_*`, disponibles a tout sorcier du Chaos) + 13 Lore of Tzeentch `_11` a `_23` (Blue Fire/Bolt of Change/Boon/Curse/Flickering Flames of Fickle Fate/Master of Fortune/Mindfire/Pink Fire/Sense the Skein/Transformation/Tzeentch's Firestorm/Tzeentch's Golden Aura/Word of Tzeentch - Talent `RULES-T0172_TZEENTCH`). Decompte verifie page par page via l'index du livre (`LIVRES/Enemy_in_Shadows_Companion.txt`) : 23 sorts reels, different de l'estimation "9+13=22" de l'audit du 22/09 (approximative, confirmation chiffree seulement). Nouveau talent `ENEMY-T0182` "Suffused with Magic" (Max 1). Portee/Cible/Duree ecrites avec les codes `(ATTR_x)`/`(BATTR_x)` deja geres par `PdfPersonnageRemplaceBonus` (`pdfpersonnage.pas:487`, couvre les 10 caracteristiques) : aucun code Pascal a modifier, saisie XML pure. Career `ENEMY-WORK106` (Cult Magus of Tzeentch) deja presente et deja reliee au talent `RULES-T0172_TZEENTCH` niveau 2 et a `ENEMY-T0181` (Tzeentch's Blessing) - rien a toucher la. **Ecarte** : les 3 creatures Daemon du chapitre (Chaos Fury, Pink Horror, Blue Horror, p.84-85), meme decision que Frostfiend/Djinn (19/09/2026) - aucune creature adverse n'est modelisee dans le programme, pas de mecanisme a etendre. Teste par Nono le 22/09/2026 (affichage WinSpell, filtre par livre). Chantier Enemy in Shadows Companion clos. Voir `A FAIRE.txt` entree "COMPANIONS DE L ENEMY WITHIN" pour les 2 ecarts restants (Horned Rat Companion, Death on the Reik Companion).

**AUDIT DES 11 DERNIERS LIVRES SUPPLEMENTAIRES (22/09/2026, sous-agents, resultats verifies par Nono a suivre)** : chantier "AUDIT DES TXT" de `A FAIRE.txt` quasi ferme. Aucun ecart sur Lords of Naggaroth, Middenheim, Parravonese/Strigany/Signaller, Salzenmund, Sea of Claws, Up in Arms, Winds of Magic (7 livres). Un bug de donnees trouve et corrige : formule de Blessures cassee sur le Goblin commun de `BOOK_GREEN_IZ_BEST.Xml` (coefficient et "+" manquants). Ecarts reels confirmes et non commences (detail dans `A FAIRE.txt`, entree "COMPANIONS DE L ENEMY WITHIN, SORTS ET ARMES MANQUANTS" et "GREEN IZ BEST") : sorts/creatures Tzeentch d'Enemy in Shadows Companion (ch.9) et de Death on the Reik Companion (Lore of Tzeentch + Boat-Mounted Weapons p.68 nouvellement releve), arsenal/magie/bestiaire skaven de The Horned Rat Companion, armes orques de Green iz Best. Seul le Rulesbook (FR+EN, ~2600 identifiants) reste a auditer sur ce chantier.
...
**SYLVANIA - THE CURSED COUNTY (20/09/2026, valide par Nono)** : `DATABASE/WFRP4/BOOK_SYLVANIA_THE_CURSED_COUNTY.Xml` (code `SYLVA`, `COMPLETE=1` depuis le 20/09/2026 : audit du TXT, plus rien a saisir hors PNJ, intrigues et tables d evenements), ajoute a `BOOKWFRP4` de `INI.TXT`. Saisis : 2 armes (`SYLVA-PROJ_ARB_01` Catechist Heavy Crossbow, `SYLVA-COMB_BASE_01` Catechist Stake), 11 objets d'auberge (suffixe « (Sylvania) »), 10 Drakenhof Sundries, 3 bateaux fluviaux (`SYLVA-TRAP_022` a `_024`, Theme `RULES-LAB_195`, Price + Carries, stats en commentaire, Enc 0). Conversions : d = SB, / = PA, GC = CO, Scare = Limited. Catechist Stake : portee lue `WEAPR_PERSO`, degats `-`, valides. Non saisis : PNJ, tables d'evenements. Fiche bateau (20/09/2026, valide) : `StructureTrapping.ProfilBateau` / `<BoatProfile>` (Crew;M voile;M rames;Man;Size;T;W), fiche = `winanimal.pas` etendu, bouton Fiche de WinEquipement et double-clic de WinPersonnage ; un bateau reste Divers (pas de type Animal, pas de rattachement). Le reste (Crewed, rattachement) attend le chantier vehicules. **Vehicules (20/09/2026, compile (0 erreur), a valider par Nono)** : profils saisis pour tous les livres en attente. `<BoatProfile>` : `SEAOF-TRAP_009` a `_026` (tableau p.93), `LUSTR-TRAP_08` (Hawkship), `DWARF-TRAP_10` a `_16` (navires) et `_17` a `_22` (engins volants) ; pour la vapeur et le vent, le Move est dans la colonne M voile avec la propulsion entre parentheses (pas de champ dedie). Nouveau champ `<VehicleProfile>` (`StructureTrapping.ProfilVehicule`, Motive Power;T;W;Strike, fiche `winanimal.pas` a 4 colonnes) : `ENEMY-TRAP_13` a `_21` (Vehicles of the Empire) et le livre partiel `COTHI` (`BOOK_SEA_WARDENS_OF_COTHIQUE.Xml`, 5 chariots, dans `INI.TXT` ; 20/09/2026, valide : + 6 betes `COTHI-TRAP_06` a `_11` (White Lion, Great Eagle, Kraken, 2 Phoenix, Swiftfeather Roc) sans Price ni Carries (absents du livre), traits optionnels et effets non saisis, + Rending Stars `COTHI-PROJ_LANC_01` 15PA, Undamaging ; Skycutter `COTHI-TRAP_12` (500CO) et Tidecutter `_13` (40CO) saisis le 20/09 comme Light Chariots, Exotic, valides ; Incandescent Spear `COTHI-COMB_HAST_01` (p.29) saisie le 20/09, valide (idem Blade of Darting Steel `COTHI-COMB_ESCR_01`, Fencing retenu faute d indication, et Bow of Anirain `COTHI-PROJ_ARC_01`, p.45) : Unique, sans Price, Durable 3/Fine 3 = fabrication, non saisis ; objets du Shadowblade, Elven Sails et Outrigger (p.63) saisis le 20/09 en 5 entrees par tranche de taille `COTHI-TRAP_14` a `_18`, Misc (LAB_196), effets non modelises, valides ; objets du Shadowblade `COTHI-TRAP_19` a `_21` (Potion of Diabolic Strength, Dark Venom, Heart of Woe) saisis le 20/09, sans prix ni Enc, effets non modelises, valides ; Gifts of Khaine non saisis, regles de MJ. Livre COTHI : plus rien a saisir hors regles de MJ et PNJ, passe a `COMPLETE=1` le 20/09/2026). Restent : qualite Crewed, rattachement (2.84). Traits de bateau (20/09/2026, compile, a valider) : descriptions en libelles `SEAOF-BOATTRAIT_<NOM>` du Sea of Claws (EN + FR), affichees par `winanimal.pas` sous le profil d un bateau (`DescriptionTraitsBateau`, `ConstPrefixeTraitBateau`) ; effets non mecanises ; Shallow Draught, Detachable Rudder et Slow non decrits dans les livres ; Grund A Dum saisi (SEAOF-TRAP_027). Menu (20/09/2026, valide) : ligne « Animals and Vehicles » (theme `RULES-LAB_195`) separee de Equipment dans `TabBibliotheque` ; `ModeEquipements` de `winequipement.pas` (meTout pour +Equipment de WinPersonnage).

**RUNES NAINES TERMINEES ET WINPERSONNAGE (20/09/2026, tout valide par Nono, dernier commit a352c5a)** : (1) Runes du Dwarf Player's Guide closes, voir §2.36 (mise a jour « CHANTIER RUNES TERMINÉ »). (2) **Avancement** : le talent de carriere a CHOIX `A/B` (ex. `RULES-T0161/NATIO-T0015_*`, Norscan Mercenary `NATIO-WORK005` niveau 1) n'etait jamais compte, `NiveauMetierTalentMax` le comparait en bloc ; corrige par la fonction locale `TalentCarriereCorrespond` (decoupe sur `SeparateurMulti`, teste chaque branche). C'etait l'origine du « bug des talents a specialisation » ; voir aussi §2.49. (3) **Choix `A / B / C` sur plusieurs lignes** dans les tables d'augmentation des talents et des competences (`winpersonnage.pas`, `ChargeAugmentation`) : `LineEnding` apres chaque « / », `RowHeights` = nb de choix x 18, cellules multilignes alignees a gauche en colonne 3 (`TabDrawCell`). **Piege** : la table des competences est TRIEE (`SortColRow`) et le tri ne deplace pas les hauteurs de ligne, elles sont reposees apres le tri ; la table des talents n'est pas triee. (4) Outils : poppler dans `C:\poppler-26.02.0` pour lire les PDF de `SAVED_CARACTERS` (voir memoire). **Reprise possible** (A FAIRE.txt) : montures et « porte par » (conception avant code), groupe, equipements mis de cote (Winds of Magic, vehicules, mines de Salzenmund).

**EQUIPEMENT DE NIVEAU COCHABLE ET (Q) = QUALITE A AJOUTER (19/09/2026, valide par Nono)** : voir Log.txt. Colonne 0 de `TabMetierEquipement` (WinPersonnage) = case a cocher (non fixe), `AjouteEquipementNiveau` a la validation ; `(Q)` -> fabrication marqueur `RULES-QUALITY_AJOUT` (`QualiteDepuisCode`), retiree par `QualiteSansMarqueur` des qu une vraie fabrication existe. Reste : afficher le marqueur dans le PDF (propose : texte "de qualite", `RULES-LAB_038`), verifier l absence de doublon avec le suffixe de `LibelleEquipement` (WinLivre/WinMetier), rattacher au catalogue les lignes texte (clothes (Q), cape(Q)...).

**HERBES DE DEATH ON THE REIK COMPANION (19/09/2026, NON COMPILE, NON TESTE)** : Trapping gagne `<Locality>` et `<Season>` (codes de libelles RULES-LAB_225 a _239 separes par des virgules ; `StructureTrapping.Localite/Saison`, `TraduireListeLibelles` dans chargetrapping.pas, lecture/ecriture xmlexportimport.pas), affiches en lecture seule dans WinEquipement si non vides. `DEATH-TRAP_01` a `_21` = les 21 herbes p.10, y compris les 4 deja au Rulebook (regle de Nono : chaque livre porte ses donnees meme si un livre officiel les a deja ; Earth Root 3CO au Companion, 5CO au Rulebook). Prix : GC -> CO, `x/-` -> PA (shilling d argent, PAS SB). Reste : compiler et tester l affichage, puis eventuellement l usage en jeu (foraging, endeavour p.9 : non modelise).

**ACCESSOIRES D ARME (19/09/2026, valide par Nono, commit ef9596b)** : voir §2 « Un accessoire est une fabrication » (`RangeBonus`, `WeaponQuality`, Accurate +10%). Bayonet fait le 20/09/2026 (balise `<AlternateWeapon>`, ligne Spear en plus sur le PDF). Affichage de la portée dans WinPersonnage : abandonné (décision de Nono, 20/09/2026), les calculs dérivés restent sur le PDF. Decisions du jour : « porte par » et groupes en pause (bonus d encombrement des montures conserve) ; creatures adverses (Frostfiend, Djinn) non gerees ; Nations of Mankind = livre de fan (OFFICIAL=2, confirme).
...
**MONTURES (19/09/2026, compile et valide par Nono sauf le chargement des deux taureaux)** : 12 animaux d Enemy in Shadows Companion (`ENEMY-TRAP_01`-`_12`) et 10 montures de Nations of Mankind (`NATIO-TRAP_01`-`_10`, les deux taureaux en valeurs maison). PDF : la somme des `<Carries>` des objets Divers de theme `RULES-LAB_195` s ajoute a l Encombrement max, avec un "(N)" en petite police (suite de la chaine des asterisques, `PdfPersonnageArmureAsterisques` renvoie `AsterisqueFinal`), dans `PdfPersonnageCreationFeldo2P` seulement. Raccourci provisoire. **Suite 20/09/2026 (valide par Nono)** : `StructureTrapping` porte `ProfilAnimal` (12 valeurs M WS BS S T I Ag Dex Int WP Fel W separees par des espaces) et `TraitsAnimal` (texte), balises `<AnimalProfile>`/`<AnimalTraits>` (`ConstXmlProfilAnimal`/`ConstXmlTraitsAnimal`, lues et ecrites dans `xmlexportimport.pas`, remises a zero a chaque entree). Saisis sur les 10 montures NATIO et 9 animaux ENEMY (Chicken/Sheep/Pig : pas de profil dans le livre ; traits Optional non saisis). Affichage : fiche `winanimal.pas` (`AfficheFicheAnimal`, formulaire construit en code sans .lfm, appelle `MiseEnFormeDesChamp`) ouverte par le bouton `ButtonFiche` de `winequipement` (visible si l objet a un profil) ; libelles `RULES-LAB_250` Profile et `_251` Traits ajoutes aux deux INTERFACE. Profil et traits volontairement absents de `TexteTrapping`. **Suite 20/09/2026 (compile, double-clic valide par Nono)** : double-clic sur une ligne animal de `TabEquipement` (WinPersonnage, `TabEquipementDblClick`, branche dans le code) ouvre `AfficheFicheAnimal`. Nouveau type d inventaire `TypeEquipAN` (`RULES-LAB_252` Animal) : pose par +Equipment si `ProfilAnimal <> ''` ; a la sauvegarde l animal reste ecrit dans le sous-chapitre Divers (format inchange), au chargement `chargepersonnage.pas` le retype AN par son profil ; le PDF (`pdfpersonnage.pas`, trois tests) le traite comme Divers. Les animaux venant de l equipement de carriere restent DI jusqu au rechargement. Bouton Fiche : libelle `RULES-LAB_253`. `winanimal.pas` declare dans le .lpi. Point de reprise : `A FAIRE.txt` ("porte par" une monture, notion de groupe).
...
**18/09/2026 — §2.80 CLOS : KENJUTSU/MARK OF THE GODS/MARTIAL ARTIST (NATIO-T0014/T0015/T0016),
MÊME TROU DE SUFFIXE `_*` QUE KNIGHTLY VIRTUE AVANT SON CORRECTIF. CORRIGÉ, COMPILÉ, TESTÉ EN
JEU ET VALIDÉ PAR NONO.** Point noté dans `A FAIRE.txt` depuis le correctif du 16/09/2026 (§2.78 suite 4)
...
**Quand Nono dit qu'il va committer (règle du 30/08/2026, précisée le 10/09/2026, revue le
23/09/2026)** — lui proposer **le récapitulatif de ce qui part dans ce commit**, sans qu'il
ait à le demander : la liste des fichiers modifiés **depuis son dernier commit**, groupés par
nature (données / code / fichiers de suivi), avec une phrase par groupe sur ce qui a changé,
et une proposition de message de commit. Cela suppose de tenir le compte, au fil de la
session, de ce qui a été livré depuis le dernier « j'ai commit » — c'est à moi de le suivre,
pas à lui de s'en souvenir.
**Depuis le 23/09/2026, je committe et pousse directement** (`git add` sur les fichiers
livrés, `git commit` avec le message proposé, `git push`) dès que Nono dit qu'il commit ou
valide le message proposé — il n'a plus besoin de le faire lui-même. Changement de règle :
Nono travaille avec TortoiseGit, pas la ligne de commande, ce qui rendait l'ancienne règle
(« c'est toujours Nono qui exécute ») contraignante sans raison en pratique. Je continue de
lui présenter le récapitulatif et le message **avant** de committer, et je ne committe/pousse
jamais sans qu'il ait dit de le faire dans cette session.
...
| **Dwarf Player's Guide** | 9 carrières nommées + ~23 autres blocs à extraire, 9 blocs d'armure, ethnies naines | **COMPLETE=1 depuis le 20/09/2026 (audit du TXT : Runic Weapons of Renown, cultes p.133-142 et regles de Grudges non saisis, sans mecanisme).**

> **Mise à jour 19/09/2026 — Dwarf Player's Guide, lot 1 fait** : `DATABASE/WFRP4/BOOK_DWARF_PLAYERS_GUIDE.Xml` (code `DWARF`, `COMPLETE=0`, déclaré dans `INI.TXT`) porte les objets p.93-97 : 9 armes de mêlée, 11 à distance/explosifs, 7 armures, 9 objets, qualité `DWARF-WEAPB01` Salvo. Conventions dans le commentaire du fichier (Scarce=LIMITED, n/a=UNIQUE, suffixe « (DPG) » quand ARCH1 a le même nom avec un autre profil, Durable/Fine du gromril = fabrication). **Reste** : voir la mise à jour suivante.

> **Mise à jour 19/09/2026 (suite) — carrières exclusives faites et validées par Nono** : 19 talents nains (`DWARF-T0001` à `T0019`, alphabétique ; Crew Commander reste `UPINA-T0173`), compétence `DWARF-COMPRUNESMITH` (Dex), spécialisations `DWARF-COMPSAVOIR_RUNES` et `DWARF-COMPSIGNES_GRIMNIR`, et les 10 carrières `DWARF-WORK01` à `WORK10` (Brewer, Hammerer, Ironbreaker, Karak Ranger, Thane, Doom Priest, Forge Priest, Hearth Priest, Runescribe, Runesmith), offertes aux nains par `DATA_SPECIE_CAREER_DIRECT` en `X` (sans jet). Lecture : le tableau d'avance est une suite d'icônes (croix=niveau 1, marteaux=2, crâne=3, bouclier=4) que l'extraction texte perd ; on rend la page en image (`pdftoppm` dans `C:\poppler-26.02.0\Libraryin`) et on la lit. Les classes se lisent dans la table aléatoire p.50. `Rune Magic` et `Master Rune Magic` sont des génériques `DWARF-T0015_*` / `DWARF-T0013_*` avec 10 spécialisations ; les runes (p.127-132) ne sont pas modélisées. **Reste** : variantes p.54-61 (niveaux alternatifs qui remplacent un niveau d'une carrière du Rulebook : greffon ou carrière dérivée, même question que les régiments de Nations, §2.37 3d-2), les 11 colonnes de forteresse de la table p.50, armes de siège (qualité Crewed à créer), sorts, runes, ethnies. Suivi : `A FAIRE.txt`.

> **Mise à jour 19/09/2026 (fin) — variantes p.54-61 faites : NI greffon NI carrière dérivée, des carrières avancées à un seul niveau.** `DWARF-WORK11` à `WORK25` (15 variantes) réutilisent le mécanisme du §2.24 : un `<Parent>` et un seul `<Level>` (le niveau remplacé), entrée `X` pour les nains ; la combo de bifurcation les propose au passage de niveau. Nouveau : `CarriereDeSuite()` (`winpersonnage.pas`) — une carrière dont `Max < niveau visé` et qui a un parent rend la main à son **premier** parent (donc Guild Engineer → niveau 3 propose Engineer, Sky Pilot, Outcast Engineer). Limite connue : Guild Engineer/Outcast Engineer (parent aussi Artillerist) et Thunderer (aussi Handgunner) retournent toujours au premier parent. Le tableau des caractéristiques de chaque variante est cumulatif, avec la caractéristique du niveau remplacé retirée (0) et celle du livre ajoutée. `MarqueNiveauEnCours()` remplace `tabNiveau.Cells[0, niveau]` (lignes séquentielles depuis le 31/08). ⚠️ **Bug racine corrigé** : `CompleteRaceMetierParEspece` dédoublonnait avec `CodeSansLivre` ; `SeparateurLivre = '-'` fait de `DWARF-WORK25` un `WORK25` qui collisionne avec `RULES-WORK25` du Rulebook, si bien que les carrières d'un livre étaient absentes pour l'ethnie de base (`RULES-RACE_DWAR`) quand leur numéro existait au Rulebook. Les clés portent maintenant les codes complets ; tout livre dont les numéros recoupent ceux du Rulebook était concerné. **Validé par Nono** : passage niveau 1→2 (Guild Engineer), ouverture du personnage et niveau 2→3 (la combo propose Engineer, Sky Pilot, Outcast Engineer). **Reste** : colonnes de forteresse p.50, armes de siège/véhicules, sorts, runes, ethnies.

> **Mise à jour 19/09/2026 (fin de journée) — ethnies, table p.50 et substitutions VALIDÉES par Nono ; 71 runes saisies et moteur d'effets côté fabrication.** Les runes (p.127-132, `DWARF-RUNE_01` à `_71`) sont des `DATA_CRAFTMANSHIP` posées par +Qualité (Max 3, Max 1 pour maîtresses et Doom). **Nouveau : les fabrications portent des modificateurs** (`ListFabricationModificateur` dans `chargefabrication.pas`, `<ModifArmour name="CodeLocalisation">n</ModifArmour>` lu dans `xmlexportimport.pas`, `PersonnageFabricationArmureModif` dans `chargepersonnage.pas` : Facteur × niveau « CODE n », pièces PORTÉES, branché aux deux PDF). Pilote validé : Rune of Stone = +1 PA partout. Pas d'export du modificateur. **Reprise :** brancher les autres cibles — caractéristiques (Fortitude +10 E, Iron +2 B, Speed +10 I via `ModifyCarac`), dégâts (Cleaving, Might, Penetrating, `ModifyDamage`), compétence (Striking, Accuracy). Bug corrigé au passage : `winfabrication.pas` passe `ButtonOk.Top` à `AdjustGridColumnsWidth` (la grille recouvrait le bouton avec 71+ lignes). Restent hors modèle : véhicules (Crewed), Longbeards p.50, règles de forge (Pride, Jealousy, Runic Flaws).
> **Mise à jour 20/09/2026 — moteur d'effets de fabrication branché sur 6 runes, TOUT VALIDÉ par Nono (sauf mention).** Chaque rune porte ses modificateurs dans `DATA_CRAFTMANSHIP` : Stone `<ModifArmour>` (PA), Fortitude `<ModifyCarac name="RULES-ATTR_T">10`, Iron `<ModifyCarac name="RULES-ATTR_Wound">2`, Speed `<ModifyCarac name="RULES-ATTR_I">10`, Cleaving `<ModifyDamage name="CC|CT">1`, Striking `<ModifySkill name="CC">10`. Chargement dans `xmlexportimport.pas` (un seul `case` : ModifArmour, ModifyCarac, ModifyDamage, ModifySkill). Somme : `PersonnageFabricationModificateur(TypeModif, Cible)` (`chargepersonnage.pas`, pièces ET armes PORTÉES, Facteur × niveau « CODE n ») et `FabricationModificateurQualite(ListeCode, TypeModif, Cible)` (`chargefabrication.pas`, un seul objet : dégâts et compétence de l'arme dans `PdfBlocArmesDonnees`, deux gabarits). Écran : colonne « Objet » de la grille d'attributs. PDF : `PdfPersonnageAttribut`, Blessures (`BonusBlessureFab`), astérisques numérotés par `PdfFabricationAsterisques` (fin de `PdfPersonnageMutationAsterisques`, clés `FAB:CodeEquipement` dans `AsterisqueParMutation`, astérisque d'arme en police 4 relevée). Feldo seulement : ligne Hardy des Blessures redécoupée (Hardy | rang | Objets | valeur | total). **Filtre par type d'objet** : balise `<Applies>` (Weapon, Armour, Misc ; vide = partout) sur les 71 runes (17/9/45), `SelectWinFabricationType` posé par WinPersonnage, `winfabrication.pas` ne liste que le applicable + ce qui est déjà posé. **Autres chantiers du jour** : case « selon mes compétences » (`RULES-LAB_240/241`, cochée par défaut, créée en code dans `winweapon.pas`, liste `SelectWinArmeCompetences` ; armes sans compétence masquées) ; fabrications sur plusieurs lignes dans `TabEquipement` (colonne 6 = affichage, colonne 7 = donnée ; `AdjustGridColumnsWidth` mesure la plus longue LIGNE si une cellule contient `LineEnding`). **Piège** : `TextRect` centré sur plusieurs lignes se superposait (toutes les lignes au même Y, cause non trouvée) — corrigé par une bande par ligne et `RowHeights` = nb de lignes × 18. **Piège** : `PFabrication` n'est pas réinitialisé entre deux éléments du parseur, tout nouveau champ doit être remis à zéro (fait pour `Applique`). **Reprise :** runes conditionnelles, voir la mise à jour suivante (Might et Grudge faites, le reste à faire) ; runes de machines (Accuracy, Penetrating...) marquées Misc, sans effet tant que les véhicules ne sont pas faits.

> **Mise à jour 20/09/2026 (soir) — effets CONDITIONNELS de fabrication : Might et Grudge faites et VALIDÉES par Nono.** Conception arrêtée avec Nono : un effet conditionnel ne modifie JAMAIS un total, il s'affiche en note. Attribut `if="CodeLibelle"` sur `<ModifyDamage>`/`<ModifySkill>` d'une fabrication, lu par `xmlexportimport.pas` dans `Filtre` de `StructureModificateur` ; `FabricationModificateurQualite` ignore toute entrée dont `Filtre <> ''` ; `FabricationEffetsConditionnels(ListeCode, Cible)` (`chargefabrication.pas`) renvoie « Rune of Might +3 DR vs larger target » (valeur × niveau, condition via `GetAllTexteLibelle`). Affichage : blocs d'explication « Crafting Bonus » (`PdfBlocDessinExplication` et l'équivalent de `PdfPersonnageCreation`), valeur PAR RUNE (libellé `RULES-LAB_244`) ; libellés `RULES-LAB_242` (plus grand) et `_243` (cible de rancune). Might : `ModifyDamage` CC/CT +3 ; Grudge : `ModifySkill` +10 et `ModifyDamage` +1, CC/CT. **Pièges** : `Format('%+d')` n'existe pas en FPC (signe à la main, `IfThen` de `StrUtils`) ; une ligne écrite à droite de l'arme se superposait aux qualités (retirée, l'explication vit dans le bloc dédié) ; `WriteText` ne réduit jamais la police, d'où `PdfEcritTient` (`pdfpersonnage.pas`) qui tient sur une ligne, utilisée dans `PdfBlocDessinExplication` seulement. **Suite, VALIDÉE par Nono le 20/09/2026 :** Resistance (`_21`, `ModifySkill` CC +10), Warding (`_35`, `ModifySkill` CC -10) et Force (`_18`, `ModifyDamage` CC valeur 0 : bonus = PA la plus haute, texte seul) ; `FabricationEffetsConditionnels` avec Cible vide = première cible trouvée par rune ; `PdfBlocDiversDonnees` reçoit `FabricationBonii` (les runes de talisman Misc alimentent le bloc Crafting Bonus) ; qualités longues du bloc Armour écrites à y+2.3 (deux lignes). **Cutting (`_03`, `LAB_248`) et Shielding (`_22`, `LAB_249`) : FAITES et VALIDÉES le 20/09/2026**, texte seul comme Force (`ModifyDamage` CC 0 avec `if=`), sans `ModifyOther` ; Cutting mêlée seulement. **CHANTIER RUNES TERMINÉ le 20/09/2026** : le bloc d'explication de `PdfPersonnageCreation` utilise maintenant `PdfEcritTient(PdfPage, 15, 74, y, texte)` (74 mm = bord du cadre Trappings, lu sur un PDF réel ; forward déclaré avant la procédure) ; les libellés `RULES-LAB_248/249` ne contiennent pas « par rune » (le code ajoute « (per rune) ») ; les qualités d'une arme À DISTANCE passent par `PdfEcrit(132, 168, y, texte, 5)` (deux/trois lignes) pour ne plus recouvrir le texte des portées (x=170) ; Feldo était déjà borné (`XGauche+114` à `+153`). **Écran WinPersonnage : décision Nono, RIEN à faire** (il sert à faire évoluer le personnage et à expliquer les valeurs de carac/compétence modifiées ; PA, Blessures, mouvement, effets conditionnels ne vivent que sur les PDF). Famille qualitative (Retribution, Daunting, Fire, Flight, Breaking, Haste, Slayer, Skalf, Snorri, Alaric) : décision Nono = ne rien modéliser.

> **Mise à jour 19/09/2026 (ethnies) — 11 ethnies naines et table p.50 faites, à tester par Nono.** `DWARF-RACE_DKARAZ/DBARAK/DAZUL/DEIGHT/DKADR/DZHUF/DHIRN/DIZOR/DNORN/DNORSE/DIMP` dans un `DATA_SPECIE` du livre, `<Ethnic>` = `RULES-SPECIE_DWARF`, profil du nain du Rulebook, compétences/talents du livre, et `SUBCHAPTER_CAREER` d100 (55 carrières, chaque colonne vérifiée 1-100). Substitutions Lustria (Lawyer→Interpreter, Coachman→Trailblazer, Pedlar→Survivalist) et Sea of Claws (Seafarer) ajoutées en `DATA_SPECIE_CAREER_CHOICE` dans CES DEUX livres (pas dans le livre nain). `DWARF-RACE_DNORSE` double `SEAOF-RACE_DNORSE`. ⚠️ Malentendu du jour : « forteresse » voulait dire l'habitat, pas les colonnes de la table. Longbeards p.50 non modélisés. **Abandonné par Nono** : armes de siège (Dwarf, Archives II). **Reste** : véhicules (qualité Crewed), sorts, runes.
...
`OFFICIAL=2`, `COMPLETE=1` depuis le 20/09/2026 (audit du TXT : les 10 montures de la p.61 sont deja saisies, NATIO-TRAP_01 a _10). Aucun fichier existant modifié, donc aucune compilation.
...
- **Un accessoire est une fabrication.** Barrel Extension, Bayonet et Telescopic Sight sont
  ...
  +10 au pourcentage de toute arme qui l'a (catalogue ou accessoire). Bayonet (20/09/2026, validé) porte `<AlternateWeapon>` (`RULES-COMB_HAST_03`, la Spear) : `PersonnageAccessoireEquipement` (chargepersonnage.pas) fabrique à la volée une ligne d'arme en plus, `Source` = `@`+code de la fabrication, encombrement 0, libellé « Spear (Bayonet) », ajoutée aux deux boucles du bloc armes du PDF. WinPersonnage n'affiche pas ces bonus. Le `Modifier` texte
...
- Code mort des montures retiré (20/09/2026). Le gabarit hors Feldo n'affiche pas les parenthèses pour armes/armures (seuls les totaux sont corrects) : DÉCISION NONO 20/09/2026, il reprend le fichier fourni par les auteurs du jeu, on n'y ajoute aucune donnée (ni fiche monture ni parenthèses), rien à faire ; groupe partagé = chantier à part.
...
- **PDF Feldo « intelligent », page 1 (20/09/2026, compilé et VALIDÉ par Nono)** : `DessinNbLigComg`/`DessinNbLigTal` sont calculés avant le dessin à partir du besoin réel (`PdfNbCompetencesGroupeesPrises`, `PdfNbTalentsAcquis`), budget de 41 lignes de données inchangé, minimum `DessinMinLignesVides` = 3, reste partagé à parts égales ; le bloc Mutations suit `DessinNbLigTal`. Restent : Mutations à lignes variables, cas où tout ne tient pas, page 2 (voir `A FAIRE.txt`).
- **PDF Feldo « intelligent », page 2 (20/09/2026, compilé et VALIDÉ par Nono avec et sans sorts)** : `PdfNbLignesExplication` compte les lignes du bloc d'explications ; une **passe à blanc** (`PdfPageBlanc`, créée par `Pages.AddPage` mais jamais ajoutée à `PdfSection`, donc non écrite : `fppdf` n'écrit que les pages des sections) rejoue `PdfBlocDiversDonnees`/`ArmesDonnees`/`ArmuresDonnees` pour obtenir les listes de bonus AVANT les cadres. Répartition : Sorts élastiques (besoin + `DessinMinLignesVides`, de 4 à 11 lignes), Armure agrandie si le texte déborde (capacité = lignes Arm + Equ + 2), reste partagé Équipement/Armes (option A). ⚠️ La page jetable doit recevoir une police (`PdfTaillePolice`) avant les blocs, sinon `EPDF: No Font was set`. À tester : personnage avec beaucoup de bonus.
- **PDF Feldo page 2, Équipement et Armure comptés (20/09/2026, compilé et VALIDÉ par Nono)** : avant le partage, `NbDivers` (Divers + Animaux) donne `NbEquNeeded = (nb + DessinMinLignesVides + 1) div 2 + 1` (deux demi-colonnes de NbLignes-1 lignes de données) et `NbArmPieces` (type détaillé ou simplifié selon `ArmureSet`) donne `pièces + 2` lignes pour l'Armure. `ExtraArm` = max(besoin des explications, besoin des pièces). L'Équipement se sert avant le partage égal, les Sorts cèdent leurs lignes vides (plancher besoin + 1, minimum 2). Bug corrigé : limite d'écriture des Divers `NbLignes * 2` → `(NbLignes - 1) * 2` (débordait de 2 entrées sous le cadre). ⚠️ Trop d'armures + de Trappings + de sorts : ça ne tient plus (décision de repli non prise). WinPersonnage : flèches `<` `>` (créées dans `FormCreate`, `PersonnageVoisinClick`) ouvrent le personnage voisin (ordre alphabétique des dossiers, via `RecherchePersonnage` + `Close`), outil de test.
- **PDF Feldo page 1, Mutations à lignes variables (20/09/2026, compilé et VALIDÉ par Nono)** : les Mutations entrent dans le budget de la colonne droite (44 lignes de données = 22 Compétences groupées + 19 Talents + 3 Mutations, total inchangé donc bas de page inchangé). `DessinNbLigMut = mutations + 1` (min 1) ; si Compétences groupées + Talents ne tiennent plus, le bloc cède sa ligne vide (jamais ses mutations, seules les plus récentes s'affichent au-delà de la capacité).
- **PDF Feldo, trappings tronqués, niveaux d'avance retirés, répartition page 2 par besoin (20/09/2026, compilé et VALIDÉ par Nono)** : (1) `PdfBlocDiversDonnees` : si `NbTotal` dépasse `(NbLignes - 1) * 2`, la dernière ligne affiche `...` (Enc total toujours calculé sur tous les objets). (2) Page 1 : les talents et compétences grisés « non acquis » ne viennent plus que du niveau de carrière en cours (talents `NiveauMetier = niveau en cours`, compétences `NivCompMetier <= niveau en cours`). (3) Page 2 : l'Armure ne grossit plus pour les explications (`ArmN = pièces + 1`, min 3) ; l'Équipement prend la hauteur des explications (`EquN >= lignes explications - ArmN - 2`), les Armes `armes + vides + 1` (min 5), les Sorts `besoin + vides` (min 4) ; le reste va à Équipement/Armes ; si tout déborde, cession dans l'ordre Sorts, Armes, Équipement (Trappings tronqués par `...`).
- **PDF Feldo page 1, talents plafonnés (20/09/2026, compilé et VALIDÉ par Nono)** : `PdfBlocTalents` limite les lignes acquises à `NbLignes - 2` (`LimiteLigne`) ; si `PdfNbTalentsAcquis` dépasse, la dernière ligne est réservée à `...` et les talents en trop ne sont pas dessinés (les grisés non plus). `PdfNbTalentsAcquis` est déclarée `forward` après `implementation`.
- **Gabarit PDF normal : aucun débordement (20/09/2026, compilé et VALIDÉ par Nono sur une fiche très chargée)** : `PdfPersonnageCreation` respecte les cadres du fichier des auteurs. Capacités déduites des coordonnées : Compétences groupées 18, Talents 20 (lignes en trop non écrites, pas de `...`) ; Armure 5, Équipement 21, Armes 7, Sorts 7 : l'objet en trop est traité normalement (encombrement, PA) mais dessiné sur une page jetable `PdfPageBlanc` (jamais ajoutée à la section) via `PageSauvee`.
- **Bloc monture sur le PDF Feldo (20/09/2026, compilé et VALIDÉ par Nono)** : `PdfBlocMontures` (page 2, à droite d'Armour Points, 50×84 mm) dessine une fiche par animal (`TypeEquipAN`) : titre + identifiant, profil M..W sur 12 colonnes, traits (2 lignes, `PdfDecoupeLignes`), « Charge N / capacité ». Fiche complète si tout tient, sinon compacte (sans traits), sinon les dernières sont omises (« ... »). `PdfLienAnimal` ajoute ` [A1]` au nom de l'animal et de ses objets confiés (Trappings, Armes, Armures). Le gabarit hors Feldo n'a pas ces éléments.
...
### 2.85 `DATA_CAREER_ADAPTATION` : substitutions de carriere par ethnie (Adapting Careers, High Elf Player s Guide p.60-62) - 20/09/2026

Bloc de LIVRE, additif, sur le modele de `DATA_CAREER_BONUS` (2.44), mais il REMPLACE au lieu de greffer. Une `<Adaptation id>` porte `<Description>`, `<Career>` (attention : la balise est `<Career>`, `ConstXmlWork`, pas `<Work>`), `<Specie>` (ethnies separees par `/`, ou Nation), `<Optional>1` (« may choose » du livre : choix du joueur, sinon imposee), `<Standing>` (bonus a tous les paliers) et des `<Level>` avec `<Order>` puis, par nature `<Skill>`/`<Talent>`/`<Trapping>`, une paire `<Remove>`/`<Add>`. Un `Remove` sans `Add` retire.

Code : constantes `ConstXmlDataCareerAdaptation` etc. (`chargeconstantes.pas`), `StructureCareerAdaptation`/`...Ligne` et `AdaptationApplicable`, `AdapterElement`, `StandingAdaptation` (`chargemetier.pas`), lecture dans `xmlexportimport.pas` (juste avant Fabrication), listes creees/videes dans `warhammersource.pas`. Application : `Personnage.MetierCompetence`/`MetierTalent` a la creation (`wincreation.pas`) et a l entree en carriere (`winpersonnage.pas`), avec `Choix` vide.

Donnees : `HELFG-ADAPT_001` a `_008` (Pit Fighter, Cavalryman, Artisan, Merchant, Seaman, Wizard, Warden, Entertainer). Ulthuan = les 10 ethnies hors Sea Elf ; Warden = les 5 Royaumes exterieurs (Tiranoc, Nagarythe, Chrace, Cothique, Yvresse). Etiquette (Guilder) du livre = talent `RULES-T0136_*` ; Track = `RULES-COMPPIST`. Facultatives : Pit Fighter et Seaman.

Valide sur les sauvegardes 3 et 4 (Ellyrion Cavalryman : Track a la place de Blackpowder au niveau 2). Standing FAIT le 20/09 (`StatutAdapte`, chargemetier.pas, appelee dans les 2 gabarits PDF de pdfpersonnage.pas ; Brass/Silver/Gold, bascule au-dela de 5) et valide sur Warden Yvresse. AFFICHAGE FAIT et valide le 20/09 : colonne de niveau des talents (NiveauMetierTalentMax, winpersonnage.pas), tableau de creation niveau 1 (wincreation.pas), WinMetier (branche par adaptation, ChargeAdaptationMetier, winmetier.pas). Decision Nono : carriere de base + note, pas de selecteur d ethnie. PDF de carriere FAIT et valide le 20/09 (PdfLignesAdaptation, pdfmetier.pas) ; TRAPPINGS FAITS le 20/09 (ChargerMetierEquipement, winpersonnage.pas : AdapterElement sur le code, type recalcule par GetTypeMetierEquipement, retrait sans ajout = pas de ligne ; compile, a valider par Nono ; creation niveau 1 non touchee, aucun trapping de niveau 1). FACULTATIVES FAITES le 20/09 (compile, a valider par Nono) : `Personnage.ChoixAdaptation` (codes d adaptation, virgules), balise `ADAPTATION_CHOICE`, `DemanderAdaptationsFacultatives` (chargemetier.pas, MessageDlg Oui/Non), transmis comme Choix a tous les AdapterElement/StatutAdapte ; creation : `ChoixAdaptationCreation` memorise par couple carriere/ethnie, ecrit dans la fiche a l etape Metier ; evolution : redemande a l entree au niveau 1 d une carriere. Non fait : modifier le choix apres coup. OPTIONS UP IN ARMS FAITES le 20/09 : Artillerist (`UPINA-WORK69`) ajoute pour `RULES-SPECIE_HELF` dans `DATA_SPECIE_CAREER_DIRECT` ; `HELFG-ADAPT_009` Light Cavalry (`UPINA-WORK76`) niveau 4 : Blackpowder -> Secret Signs (Scout), pistolet -> texte libre `magic weapon` (munitions gardees, le livre ne les cite pas). Archer, Camp Follower et Light Cavalry etaient deja ouverts aux Hauts Elfes par Up in Arms lui-meme. Non porte : restriction Soldier. Adapting Careers est donc complet, sauf ces non-portes.

Bug ancien corrige au passage : `NbMaxCompetence` = 12 < 13 competences de race des Ellyrion (case Ride cochee au hasard) ; `CompetenceRaceStates` redimensionne a chaque reconstruction.
...
**Fait le 21/09, teste et valide par Nono le 22/09** : Ogre (Archives II), Elfe noir (Lords of Naggaroth : age et 2 tirages d yeux supposes comme les elfes du Rulebook), 5 peaux-vertes (Green iz Best : yeux/cheveux en ligne unique, pas de table dans le livre). Donnees seules.
...
**Prefixe + suffixe des nains (fait apres)** : tables d100 du Character Pack (p.11) posees sur le Nain du Rulebook (parties `Prefix` 75 lignes, `Suffix` 63 lignes + 13 doublons M/F pour les suffixes en a/i : « -ra/i » donne `ri` (M) ou `ra` (F)). Notations du livre traduites : `X/Y` -> variantes `X/Y` (remplace la derniere lettre : Rag/k = Rag ou Rak, Sim/n = Sim ou Sin, dim/n = dim ou din), `Bra(g/k)` -> `Brag/Brak`, `(x)` facultative. `TireNom` tire desormais UN modele au hasard parmi ceux du sexe demande : le Nain a donc deux modeles par sexe (prenom du DPG, ou prefixe + suffixe), un tirage sur deux chacun ; le nom du pere suit le meme principe (`Prefix+Suffix:M+'sson'`).

**Noms nobles (fait apres)** : `<NamePattern noble="1">"Forename 'von_'+Place"</NamePattern>` sur l humain du Rulebook + 14 lieux du Reikland cites dans le Rulebook (Altdorf, Auerswald, Bogenhafen, Carroburg, Dunkelberg, Eilhart, Grunburg, Helmgart, Kemperbad, Schilderheim, Stimmigen, Tahme, Ubersreik, Weissbruck) : « von » + lieu REMPLACE le nom de famille (Character Pack p.10 ; le livre ne donne pas de noms de maison). `TireNom(CodeRace, Sexe, Noble)` : un noble prend d abord les modeles marques `noble`, a defaut les ordinaires ; dans un texte fixe `_` = espace ; la majuscule forcee du 1er caractere est supprimee (les tables portent leurs majuscules). UI : `CheckBoxNoble` + `LabNoble` (TEdit desactive faisant office de legende, comme `LabNomPersonnage`) sur l onglet Nom, libelle `RULES-LAB_271`. Le Rulebook ne contient AUCUNE liste de lieux (la carte du Reikland est une image) : liste courte volontaire, a etendre si un gazetteer est saisi.
...
**Tileens (fait apres)** : `UPINA-RACE_HTIL` (`BOOK_UP_IN_ARMS.Xml`, et non le bloc `UPINA-RACE_HTIL` de Nations of Mankind qui n ajoute que des carrieres) porte les listes d Up in Arms p.56 : 33 prenoms M, 33 F, 33 noms de famille (tirage uniforme), modeles `Forename Surname` M et F. Donnees seules. **Estaliens : aucune source** dans les livres (Nations of Mankind ne donne aucun nom), rien de saisi.

**Nordland et Ostland (fait apres, demande de Nono)** : `MIDDE-RACE_HNORD` (Middenheim) et `NATIO-RACE_HOSTL` (Nations of Mankind) portent 8 prenoms M et 8 F du Character Pack p.8 (wastelanders : Filip, Anders, Joachim, Astrid, Ingrid ; influence norse : Axel, Bengt, Rolf, Holger, Knud, Frida, Sigrid, Solveig, Erika, Karin, Kirsten), sans modele propre : modeles et noms de famille du Reiklandais, dont les patronymes -sson / -sdottir (courants dans le nord). Wastelander (`NATIO-RACE_HWEST`) : les memes 16 prenoms (decision de Nono : les 5 wastelanders seuls se repetaient trop vite ; le livre place les prenoms norses dans l « extreme nord »).
...
**Donnees** : `<DATA_STARSIGN>` dans `BOOK_ARCHIVES_OF_THE_EMPIRE_II.Xml` (ids `ARCH2-STAR01` a `20`) : `Range`, `Description`, `Title`, `Classical`, `Ascendant`, `Dates`, `God`, `Appearance`, `Personality` (texte de personnalite, ajoute 25/09/2026, purement informatif), `Modifier name="RULES-ATTR_xx"`, `Talent`, `Variant range="1-3"` (Witchling Star).

**Code** : `chargesigneastral.pas` (`StructureSigneAstral`, `StructureSigneEffet`, `ChercheSigneAstral`, `TireSigneAstral`, `SigneAstralTireVariante`, `SigneAstralAttributModif`, `SigneAstralTexteEffets`, `SigneAstralTextePdf`, `ResumeSigneAstral`) ; lecture/export dans `xmlexportimport.pas` (export seulement dans le fichier anglais) ; constantes `ConstXmlSigne*`. `Personality` (`ConstXmlSignePersonnalite`) affiche en fin de `ResumeSigneAstral`, apres les effets et talents. Fiche : `Personnage.SigneAstral` et `SigneAstralVariante`, balises `StarSign` / `StarSignVariant` (chargepersonnage.pas). Effets : `PdfPersonnageAttribut` (accesseur unique, §2.52) et `PersonnageMutationTalent` (talent, meme mecanisme que les mutations, §2.54). Ecran : onglet Nom de `wincreation` (`ChargeSignesAstraux`, `AppliqueSigneAstral`, `SigneTire`, remise a zero au choix de l ethnie) ; `EditSigne` dans `winpersonnage` ; ligne dans les deux blocs Talents du PDF. Libelles `RULES-LAB_272` a `274`.

**Sans le livre charge** : liste vide, onglet masque ; sur une fiche existante les effets disparaissent en silence, d ou l ajout du livre du signe a `LivresObligatoires` (le mecanisme des livres manquants prend le relais).

**Pieges** : `WinPersonnage` : les positions viennent de `AjustePositionTables`, pas du .lfm ; la ligne des talents (`LabTabTalent.Top`) passe sous la colonne de gauche quand elle est plus haute. `warhammersource.pas` est en CRLF, les autres fichiers en LF.

**Ligne de detail et regle des lignes vides (WinPersonnage, meme jour)** : `LigAttSigne` (apres `LigAttObjet`) dans `TabAttribut`, 16 lignes (`LigAttBase` .. `LigAttAsterisc` decales de 1) ; `AjusteDetailsCalcul` (`winpersonnage.pas`) : une ligne ou colonne de detail (Mutation, Carriere, Objet, Signe ; colonnes de competence 35, 40, Travail, Mutation, Carriere, Appartenance) ne se montre que si « Calcul » est coche ET qu elle porte une valeur non nulle ; appelee par `CheckBoxCalculChange` et en fin de `CalculTotaux` (qui refait alors `AjustePositionTables`). Race, Lance, Talent restent toujours affichees. **Piege** : `TabAttribut` a ~15 colonnes ET ~16 lignes ; deux boucles utilisaient `RowCount - 1` comme borne de COLONNE et `EffaceDonnee(TabAttribut, 15)` fixait le nombre de lignes : a corriger si on ajoute encore une ligne.

**PDF maison (meme jour)** : `PdfBlocEntete` prend une 5e ligne « Signe astral » (parametre `AnnotSigne`, `NbLignes` = 4 + 1 si un signe) ; `PdfSigneAstralAsterisque` (`pdfpersonnage.pas`, appelee en fin de `PdfPersonnageMutationAsterisques`) numerote UNE asterisque par signe qui modifie une caracteristique (« +2 (N) » sur la caracteristique, « (N) » sous la cle `SIGNE` de `AsterisqueParMutation`, lue par l en-tete). La ligne du signe est retiree du bloc Talents maison, gardee sur le gabarit du livre. Cellule des cheveux de l en-tete : `XValCheveux := XSep2 + 12`.

**Talent du signe (21/09, compile, a tester)** : ECRIT dans `CreationTalent` par `AjouteTalentSigne` (wincreation, phase 9, avant la sauvegarde) ; un talent deja pris gagne un niveau (une seule ligne), un talent generique sans specialisation choisie n est pas donne. `PersonnageMutationTalent` ne s en occupe plus (sinon doublon) : seules les caracteristiques du signe restent calculees au vol. `Personnage.SigneAstralTalent` (balise `StarSignTalent`) porte la specialisation choisie, jusqu a la sauvegarde. Les fiches de test sauvegardees avant ce changement perdent le talent du signe. `wincreation` : bouton `ButtonSigneSpe` (cree en code sous `MemoSigne`, reduit de 36 px) ouvre `TWinSpecialisations` ; le joueur choisit toujours, meme apres un tirage. `SigneAstralTalentGenerique` (chargesigneastral). Libelles `RULES-LAB_275/276`. Recap de creation : bonus du signe ajoute. Le changement de signe apres la creation n existe pas dans les regles (p.39).
...
**Texte de personnalite (25/09/2026, compile, a tester)** : les 20 `Personality` saisis (p.39-46), purement informatif, aucun code de calcul. Etapes optionnelles p.50 (signe ascendant, Celestial Mansions) : decision de Nono, NON implementees (purement roleplay, aucun effet mecanique) - voir A FAIRE.txt.

**Reste** : voir A FAIRE.txt (objets magiques Archives II p.51-66, autres livres).
...
**Interface dediee (22/09, testee et validee par Nono)** : fenetre `WinOptionRegle` (bouton "Optional rules"/"Regles optionnelles" sous Panel3, TMenu) listant TOUTES les regles optionnelles de tous les livres charges (livre / option / case a cocher), avec l explication du livre affichee dans un memo au clic sur une ligne - demande de Nono 22/09/2026 ("une fenetre a part avec le livre, l option et une explication"), l explication venant des DONNEES du livre (pas du Pascal) :
- Nouveau bloc XML par livre, `<DATA_OPTION_RULE>` (`<OptionRule id="RULES-OPTR_01"><Code>"QUICKARMOUR"</Code><Description language="...">...</Description><Explanation language="...">...</Explanation></OptionRule>`) - ajoute dans `BOOK_RULESBOOK.Xml` et `BOOK_RULESBOOK_FRANCAIS.Xml` (texte de Quick Armour p.301 copie du PDF anglais ; PAS traduit en francais, meme dette que les Description d ArmorSimp deja non traduites dans ce livre).
- `chargeoptionregle.pas` (nouvelle unite) : `StructureOptionRegle` (Livre, CodeId, CodeOption, Libelle, Explication), `ListOptionRegle`. Chargement dans `xmlexportimport.pas` juste apres le bloc ArmorSimp - toujours charge (definitions), seule une passe anglaise alimente la liste (meme limite que ListArmureSimplifiee : pas de re-localisation dynamique via `Traduit()` pour l instant).
- `winoptionregle.pas` (nouvelle unite) : fenetre `TWinOptionRegle`, ENTIEREMENT construite en code (pas de .lfm, pour eviter d ecrire un fichier de resource a la main). Grille `TabOption` avec 3 colonnes ajoutees via `GridAjouteColonne` (memes indices retournes que `ColLivreSel` pour `TabLivre` - la colonne 0 d une grille est FIXE, jamais selectionnable au clic, piege trouve en testant : la case ne reagissait a rien tant que le code utilisait Cells[0,...]/Col=0). Persistance via `SauveIniOptions()` (chargeconstantes.pas) : reecrit SEULEMENT les lignes `OPTION<Livre>=` du .INI en preservant le reste tel quel, pour eviter une dependance circulaire (WinOptionRegle ne peut pas importer warhammersource.pas/TMenu.SauveIni, qui depend lui-meme de tout l ecran principal).
- Bouton "Optional rules" (`RULES-LAB_277`, sous Panel3 dans `TMenu`) : cree en CODE (pas dans le .lfm), mais son `.Caption` doit etre pose APRES le chargement d INTERFACE.Xml (`ListTexte` cree ~L1559, rempli ~L1619) - appeler `GetTexteLibelle` trop tot (dans le bloc `FormCreate` d origine, avant ce chargement) provoquait un crash "ACCESS VIOLATION" au demarrage (`generics.collections.pas`), trouve par Nono en lancant depuis l IDE.
- Nouveaux libelles `RULES-LAB_277` a `280` (Optional rules / Active / Explanation / Close) dans `INTERFACE.Xml` et `INTERFACE_FRANCAIS.Xml`.
- Deux bugs supplementaires trouves en testant et corriges : (1) `TabOption.ColCount := 1` protege par un `if ColCount < 2` qui ne se declenchait jamais (ColCount par defaut d une grille neuve = 5, jamais < 2) - les 3 colonnes ajoutees s empilaient sur les 5 par defaut (colonnes en trop, en-tetes vides a l ecran) ; (2) `WinCharger` appele DEUX FOIS (une fois manuellement dans le constructeur, une fois via `AfterConstruction`/`DoCreate`/`OnCreate` juste apres) - le 2e appel replantait sur `ColCount := 1` (interdit une fois que `Columns` est deja active par le 1er appel, `EGridException`).

**Etendu aux 2 autres options connues (22/09/2026)** : memes principes (bloc `<DATA_OPTION_RULE>` dans le livre, garde `OptionDesactivee` a l ajout dans la liste memoire) :
- Signes astraux (`ConstOptionStarSign = 'STARSIGN'`) : `BOOK_ARCHIVES_OF_THE_EMPIRE_II.Xml` (`ARCH2-OPTR_01`) ; garde ajoutee dans `xmlexportimport.pas` au `ListSigneAstral.Add` (le chargement des EFFETS du signe, `AjouteEffetSigneAstral`, N EST PAS garde - reste appele meme option desactivee, mais reste orphelin/inoffensif puisque le signe lui-meme n est plus dans la liste).
