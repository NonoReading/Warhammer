...
uses
  Classes, SysUtils, ChargeConstantes, ChargeMetier, Generics.Collections, ChargeTexte, ChargeRace;
...
Function VerifieRaceMetier(RaceChoisie: String; MetierActuel: String; MetierAVerifier:String):Boolean;
  ...
  begin
    For PRaceMetier in ListRaceMetier do
      if (PRaceMetier.CodeRace = RaceChoisie) and (PRaceMetier.CodeMetier = MetierAVerifier) then
        begin
...
Function MetierRaceCourt(MetierActuel: String):String;
  ...
  begin
    For PRaceMetier in ListRaceMetier do
      if (PRaceMetier.CodeMetier = MetierActuel) then
        begin
...
Function MetierRaceNbSuivant(RaceEnCours:String; MetierEnCours: String):Integer;
  ...
  begin
    For PRaceMetier in ListRaceMetier do
      if (PRaceMetier.CodeRace = RaceEnCours) then
        begin
