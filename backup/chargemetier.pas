...
Function CheminMetierImage(CodeMetier: String): String;
  ...
  begin
    for PMetier in ListMetier do
      if CompareRechercheValeur(PMetier.CodeMetier, CodeMetier) then
        Begin
          ResTrans  := GetCurrentDir+StringReplace(ConstCheminImageMetier, ConstLivre, PMetier.Livre, [rfReplaceAll])+CodeMetier+ConstTransparent+'.PNG';
          ResNormal := GetCurrentDir+StringReplace(ConstCheminImageMetier, ConstLivre, PMetier.Livre, [rfReplaceAll])+CodeMetier+'.PNG';
